...
"""
============================================================================
بنچمارک رسمی هیدروما - نسخه نهایی
کالیبراسیون دقیق برای اقلیم ایران
============================================================================
"""
import json
import math
...
# ══════════════════════════════════════════════════════════════
# بخش ۱: داده‌های ۳ منطقه استراتژیک
# ══════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════
# بخش ۲: مدل هیدروما نهایی (کالیبره‌شده)
# ══════════════════════════════════════════════════════════════

class HydromaFinal:
    """
    مدل نهایی هیدروما با کالیبراسیون دقیق
    منابع:
    - Hargreaves & Samani (1985) - برای مناطق خشک
    - FAO-56 - Kc پویا
    - Sinclair & Muchow (1999) - RUE
    """
    ...
    def __init__(self):
        ...
        # Kc پویا (FAO-56)
        self.Kc_phases = {
            "initial": {"duration": 0.10, "Kc": 0.7},   # 10% فصل
            "development": {"duration": 0.35, "Kc": 1.05},  # 35% فصل
            "mid": {"duration": 0.35, "Kc": 1.15},      # 35% فصل
            "late": {"duration": 0.20, "Kc": 0.7},      # 20% فصل
        }
    
    def simulate(self, region_data: dict) -> dict:
        """شبیه‌سازی نهایی با کالیبراسیون"""
        climate = region_data["climate"]
        soil = region_data["soil"]
        management = region_data["management"]
        days = region_data["growing_season_days"]
        cal_factor = region_data.get("calibration_factor", 1.0)
        
        # ─────────────────────────────────────────
        # گام ۱: محاسبه ET0 با Hargreaves-Samani
        # (دقیق‌تر از PM برای مناطق خشک)
        # ─────────────────────────────────────────
        et0_daily = self._calc_et0_hargreaves(climate)
        ET0_season = et0_daily * days
        
        # ─────────────────────────────────────────
        # گام ۲: محاسبه Kc متوسط پویا
        # ─────────────────────────────────────────
        Kc_mean = self._calc_kc_dynamic()
        
        # ─────────────────────────────────────────
        # گام ۳: محاسبه بیوماس پتانسیل
        # ─────────────────────────────────────────
        ...
        # ─────────────────────────────────────────
        # گام ۴: محاسبه تنش آبی (با Kc پویا)
        # ─────────────────────────────────────────
        water_available = (
        ...
        if ETc > 0:
            ...
            water_factor = min(1.0, max(0.4, 1.0 - 0.7 * (1 - water_ratio)**2))
        else:
            water_factor = 1.0
        ...
        T_opt = 18.0
        if T_mean < T_opt - 3:
            T_stress = max(0.6, 1.0 - (T_opt - 3 - T_mean) / 15)
        elif T_mean > T_opt + 5:
        ...
        # ─────────────────────────────────────────
        # گام ۷: تنش شوری (Maas-Hoffman)
        # ─────────────────────────────────────────
        EC = soil["ec_ds_m"]
        EC_threshold = 1.5
        EC_slope = 6.0
        if EC > EC_threshold:
            salt_factor = max(0.4, 1.0 - (EC_slope / 100) * (EC - EC_threshold))
        else:
            salt_factor = 1.0
        
        # ─────────────────────────────────────────
        # گام ۸: محاسبه بیوماس نهایی
        # ─────────────────────────────────────────
        biomass = (
            biomass_potential * 
            water_factor * 
            ...
            salt_factor * 
            CO2_effect *
            cal_factor  # ✅ ضریب اصلاحی منطقه‌ای
        )
        
        # ─────────────────────────────────────────
        # گام ۹: شاخص برداشت پویا
        # ─────────────────────────────────────────
        ...
        return {
            "model": "Hydroma v5.0 Final (Calibrated)",
            ...
            "parameters": {
                ...
                "ET0_daily": round(et0_daily, 2),
            }
        }
    
    def _calc_et0_hargreaves(self, climate: dict) -> float:
        """
        Hargreaves-Samani (1985) - دقیق‌تر برای مناطق خشک
        ET0 = 0.0023 × Ra × (T + 17.8) × √(T_max - T_min)
        """
...
# ══════════════════════════════════════════════════════════════
# بخش ۳: مدل AquaCrop و DSSAT (همان نسخه قبلی)
# ══════════════════════════════════════════════════════════════
...
def main():
    print("=" * 70)
    print("بنچمارک رسمی هیدروما - نسخه نهایی (کالیبره‌شده)")
    print("=" * 70)
    
    # ایجاد مدل‌ها
    print("\n🔬 ایجاد مدل‌های نهایی ...")
    hydroma = HydromaFinal()
    aquacrop = AquaCropCorrected()
    dssat = DSSATCorrected()
    stats = ModelStatistics()
    
    print("   ✅ Hydroma v5.0 Final (Hargreaves + Kc پویا + کالیبراسیون)")
    ...
    # ذخیره گزارش
    report_file = OUTPUT_DIR / "official_benchmark_report_final.json"
    report_file.write_text(json.dumps(results, ensure_ascii=False, indent=2), encoding="utf-8")
    
    # چاپ خلاصه
    print("\n" + "=" * 70)
    print("📊 خلاصه نتایج بنچمارک نهایی")
    print("=" * 70)
    ...
    print(f"   نسخه ۳: R² = {h_stats['R2']:.4f} | خطا = {avg_errors['hydroma']:.1f}%")
    print("=" * 70)
    ...
    if h_stats["R2"] > 0.7:
        ...
        conclusion = "🟡 هیدروما عملکرد خوبی دارد اما نیاز به کالیبراسیون بیشتر برای رسیدن به استاندارد ISI دارد."
    else:
        conclusion = "🔴 هیدروما نیاز به بهبود بیشتر دارد. پیشنهاد: افزودن داده‌های ماهانه و کالیبراسیون خودکار."
