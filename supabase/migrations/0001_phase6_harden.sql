...
drop policy if exists "landscapes_public_read" on public.platform_landscapes;
create policy "landscapes_public_read" on public.platform_landscapes
    for select to anon, authenticated using (true);

drop policy if exists "standards_public_read" on public.standards;
create policy "standards_public_read" on public.standards
    for select to anon, authenticated using (true);

drop policy if exists "badges_public_read" on public.platform_badges;
...
drop policy if exists "profiles_own_read" on public.platform_profiles;
create policy "profiles_own_read" on public.platform_profiles
    for select to authenticated using (auth.uid() = id);
drop policy if exists "profiles_own_insert" on public.platform_profiles;
...
drop policy if exists "carbon_projects_own_read" on public.platform_carbon_projects;
create policy "carbon_projects_own_read" on public.platform_carbon_projects
    for select to authenticated using (auth.uid() = owner_id);
drop policy if exists "carbon_projects_own_insert" on public.platform_carbon_projects;
create policy "carbon_projects_own_insert" on public.platform_carbon_projects
    for insert to authenticated with check (auth.uid() = owner_id);
drop policy if exists "carbon_projects_own_update" on public.platform_carbon_projects;
