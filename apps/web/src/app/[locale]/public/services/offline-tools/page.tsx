...
export default async function OfflineToolsPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations('public.services.offlineTools');

  const statusResult = await publicApi.services.status('offline-tools');
