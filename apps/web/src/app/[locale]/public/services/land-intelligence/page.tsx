...
export default async function LandIntelligencePage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations('public.services.landIntelligence');

  const statusResult = await publicApi.services.status('land-intelligence');
