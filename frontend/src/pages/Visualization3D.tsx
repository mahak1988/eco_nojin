...
import {
  Mountain, Map, Layers, Loader2, Info,
  Droplet, Leaf, Play, RefreshCw, Maximize2,
  Ruler, TrendingUp, Activity, Eye, EyeOff,
  Download, Share2, Camera, Compass, Crosshair,
  Thermometer, Sun, Cloud, Wind, Zap,
  Satellite, Globe, Box, PieChart as PieChartIcon
} from 'lucide-react';
  ...
  for (let i = 0; i < positions.count; i++) {
    ...
    if (y < data.height && x < data.width) {
      ...
      if (hasSlope) {
        ...
        const dx = x > 0 && x < data.width - 1 
          ? data.elevation[y][x + 1] - data.elevation[y][x - 1]
          : 0;
        const dy = y > 0 && y < data.height - 1
          ? data.elevation[y + 1][x] - data.elevation[y - 1][x]
          : 0;
...
// ============== Measurement Markers ==============
function MeasurementMarkers({ points, lines }: { 
...
export default function Visualization3D() {
  ...
  const [layers, setLayers] = useState<LayerConfig[]>([
    { id: 'elevation', name: 'Elevation', icon: Mountain, color: '#8b5cf6', enabled: true, opacity: 1 },
    { id: 'water', name: 'Water', icon: Droplet, color: '#3b82f6', enabled: true, opacity: 0.5 },
    { id: 'vegetation', name: 'Vegetation', icon: Leaf, color: '#10b981', enabled: true, opacity: 1 },
    { id: 'ndvi', name: 'NDVI (Satellite)', icon: Satellite, color: '#16a34a', enabled: false, opacity: 1 },
    { id: 'carbon', name: 'Carbon Stock', icon: Leaf, color: '#22c55e', enabled: false, opacity: 1 },
    { id: 'moisture', name: 'Soil Moisture', icon: Droplet, color: '#06b6d4', enabled: false, opacity: 1 },
    { id: 'flood', name: 'Flood Zones', icon: Droplet, color: '#dc2626', enabled: false, opacity: 1 },
    { id: 'capability', name: 'Land Capability', icon: Map, color: '#f59e0b', enabled: false, opacity: 1 },
    { id: 'slope', name: 'Slope Analysis', icon: TrendingUp, color: '#ef4444', enabled: false, opacity: 1 },
  ]);
  ...
  // Statistics
  const totalDistance = measurementLines.reduce((sum, [a, b]) => 
    sum + a.distanceTo(b), 0) * 100; // to meters
  ...
  return (
    <div className="admin-page-container">
      ...
      <div className="grid-3col" style={{ marginBottom: '20px' }}>
        ...
        <div className="glass-card" style={{ padding: '16px' }}>
          ...
          {measurePoints.length > 0 && (
            <div style={{ marginTop: '12px', padding: '8px', background: 'var(--bg-hover)', borderRadius: '6px', fontSize: '12px' }}>
      ...
      {/* 3D Canvas */}
      <div className="glass-card" style={{ height: '700px', padding: '0', overflow: 'hidden', position: 'relative' }}>
        <Canvas shadows>
          <PerspectiveCamera 
            makeDefault 
            position={
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
            <Globe size={14} />
            <span>Location: {terrain.lat?.toFixed(2)}°, {terrain.lon?.toFixed(2)}°</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
            <Mountain size={14} />
            <span>Elevation: {Math.round(terrain.minElevation)}m - {Math.round(terrain.maxElevation)}m</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <Box size={14} />
            <span>Resolution: {terrain.width}×{terrain.height}</span>
          </div>
        </div>
        ...
        <div className="glass-card" style={{ marginTop: '20px', padding: '20px' }}>
          ...
          <div style={{ height: '200px', position: 'relative' }}>
            <svg width="100%" height="100%" viewBox="0 0 800 200" preserveAspectRatio="none">
              ...
              {/* Points */}
              {elevationProfile.filter((_, i) => i % 5 === 0).map((p, i) => {
                ...
                return (
                  <g key={i}>
                    ...
                    <text x={x} y={y - 10} textAnchor="middle" fill="var(--text-muted)" fontSize="10">
          ...
          <div style={{ marginTop: '12px', fontSize: '12px', color: 'var(--text-muted)', display: 'flex', justifyContent: 'space-between' }}>
            <span>Distance: {elevationProfile[elevationProfile.length - 1].distance.toFixed(1)}m</span>
            <span>
              Min: {Math.round(Math.min(...elevationProfile.map(p => p.elevation)))}m | 
              Max: {Math.round(Math.max(...elevationProfile.map(p => p.elevation)))}m |
              Avg: {Math.round(elevationProfile.reduce((s, p) => s + p.elevation, 0) / elevationProfile.length)}m
            </span>
          </div>
        </div>
      )}
