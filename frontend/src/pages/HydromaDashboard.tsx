...
import { MaterialsCard } from '../components/hydroma/MaterialsCard';
import { InsuranceCard } from '../components/hydroma/InsuranceCard';
import { AccountingCard } from '../components/hydroma/AccountingCard';
import { BlockchainCard } from '../components/hydroma/BlockchainCard';
import { TourismCard } from '../components/hydroma/TourismCard';
...
const DashboardScene3D = React.lazy(() =>
  import('../components/hydroma/DashboardScene3D').then((m) => ({ default: m.DashboardScene3D }))
);
    ...
    <div className="hydroma-bg" style={{ minHeight: '100vh' }}>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          ...
          <Button variant="ghost" onClick={logout} icon={<LogOut size={15} />}>
            خروج
          </Button>
        </div>
      </header>
      ...
      >
        ...
        >
          <h1
            style={{ fontSize: '1.9rem', fontWeight: 800, color: '#fff', marginBottom: '0.5rem' }}
          >
            خوش آمدی، {user?.name} 🌍
          </h1>
      ...
      >
        ...
        <div
          ...
          style={{ marginBottom: '1.5rem', alignItems: 'start', gap: '1.2rem' }}
        >
          <RealLandSummaryCard
            onLoaded={setRealLand}
            onCoordsChange={(lat: number, lon: number) => setCoords({ lat, lon })}
          />
        ...
        <div style={{ marginBottom: '1.5rem' }}>
          ...
          >
            ...
            {sceneOpen && (
              <Button variant="ghost" onClick={() => setSceneOpen(false)} icon={<X size={14} />}>
                بستن
              </Button>
            )}
          </div>
          {sceneOpen ? (
            ...
            >
              برای باز کردن شبیه‌ساز سه‌بعدی هر ماژول، دکمه «سه‌بعدی» کنار عنوان ماژول را بزنید —
              صحنه با داده واقعی زنجیره (پارتو، عمق سیلاب، استخرهای کربن، عملکرد محصول) تغذیه
              می‌شود.
            </div>
          )}
        </div>
        ...
        `r`n`r`n {/* Real Supabase landscapes (Phase 6-B) */}`r`n{' '}
        <SupabaseMapCard lat={coords.lat} lon={coords.lon} />
        `r`n`r`n {/* Marketplace catalog on Supabase (Phase 6-B) */}`r`n <MarketplaceCard />
        `r`n`r`n {/* LMS (Phase 6-C) */}`r`n <LmsCard />
        `r`n`r`n {/* Audit & credits (Phase 7) */}`r`n <AuditCard />
        `r`n`r`n {/* Profile & Admin (Phase 8-A) */}`r`n <ProfileCard />
        `r`n <AdminCard />
        `r`n`r`n {/* Phase 8-B: pro simulators */}`r`n <LightSimCard />
        `r`n <DroughtCard />
        `r`n <ClimateCard />
        `r`n`r`n {/* Phase 8-C: spider firewall + AI advice */}`r`n <SecurityCard />
        `r`n <AiAdviceCard />
        `r`n`r`n{' '}
        {/* Backend business modules: materials / insurance / accounting / blockchain / tourism */}
        `r`n <MaterialsCard />
        `r`n <InsuranceCard />
        `r`n <AccountingCard />
        `r`n <BlockchainCard />
        ...
        >
          <Link to="/simulator">
            <Button variant="primary">مرکز شبیه‌سازهای علمی</Button>
          </Link>
          <Link to="/virtual-lab">
            <Button variant="secondary">آزمایشگاه مجازی زمین</Button>
          </Link>
        </div>
      </main>
    </div>
