              ...
              <Reveal key={phase.tag} delay={index * 0.08}>
                <div
                  className={`glass glass-hover h-full rounded-3xl p-7 ${
                    index === 0 ? 'border-[var(--color-leaf-500)]/30 bg-[var(--color-leaf-500)]/8' : 'border-[var(--color-sand-500)]/25 bg-[var(--color-sand-500)]/6'
                  }`}
                >
                  <div className="flex flex-wrap items-center gap-2">
                    <span
                      className={`rounded-full px-3 py-1 text-[11px] font-bold ${
                        index === 0 ? 'bg-[var(--color-leaf-500)]/15 text-[var(--color-leaf-300)]' : 'bg-[var(--color-sand-500)]/15 text-[var(--color-sand-300)]'
                      }`}
                    >
                      {phase.tag}
                    </span>
                    <span
                      className={`rounded-full px-3 py-1 text-[11px] font-bold ${
                        index === 0 ? 'bg-[var(--color-aqua-500)]/15 text-[var(--color-aqua-300)]' : 'bg-white/8 text-[var(--color-night-200)]/60'
                      }`}
                    >
                      {phase.status}
                    </span>
      ...
      <section className="px-4 py-10 sm:px-6" id="chain">
        <div className="mx-auto flex max-w-6xl flex-col gap-8">
          ...
          <ol className="relative grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
            <div
              className="absolute inset-x-10 top-9 hidden border-t-2 border-dashed border-[var(--color-leaf-500)]/25 lg:block"
              aria-hidden
            />
            {c.chain.map((step, index) => (
              <Reveal key={step.title} delay={index * 0.07}>
                <li className="glass glass-hover relative flex h-full flex-col gap-2 rounded-3xl p-5">
                  <span className="ring-glow inline-flex h-9 w-9 items-center justify-center rounded-full bg-[var(--color-leaf-500)] text-xs font-extrabold text-[var(--color-night-950)]">
        ...
        <div className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-2">
          <Reveal>
            <div className="glass h-full rounded-3xl p-7">
              <h2 className="flex items-center gap-2 text-lg font-extrabold text-[var(--color-night-100)]">
                <ShieldCheck className="h-5 w-5 text-[var(--color-leaf-400)]" aria-hidden />
                {c.commitmentsTitle}
              </h2>
          ...
          <Reveal delay={0.08}>
            <div className="ring-glow flex h-full flex-col justify-center gap-3 rounded-3xl bg-gradient-to-b from-sand-500/15 to-night-900 p-7">
              <h2 className="flex items-center gap-2 text-lg font-extrabold text-[var(--color-night-100)]">
                <Sparkles className="h-5 w-5 text-[var(--color-sand-300)]" aria-hidden />
                {c.gateTitle}
              </h2>
              <p className="text-sm leading-8 text-[var(--color-night-200)]/70">{c.gateBody}</p>
              <div className="mt-2 flex flex-wrap gap-2 text-xs font-bold">
                <Link to="/terms" className="rounded-full bg-white/8 px-4 py-1.5 text-[var(--color-night-200)]/70 hover:bg-white/15">
                  {t.footer.legalLinks.terms}
                </Link>
                <Link to="/transparency" className="rounded-full bg-white/8 px-4 py-1.5 text-[var(--color-night-200)]/70 hover:bg-white/15">
                  {t.footer.quickLinks.transparency}
                </Link>
                <Link to="/carbon" className="rounded-full bg-white/8 px-4 py-1.5 text-[var(--color-night-200)]/70 hover:bg-white/15">
                  {dir === 'rtl' ? 'کربن' : 'Carbon'}
                </Link>
              </div>
            </div>
