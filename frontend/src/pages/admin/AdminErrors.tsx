...
export default function AdminErrors() {
  ...
  const filteredErrors =
    filter === 'all' ? errors : filter === 'open' ? openErrors : resolvedErrors;
          ...
          <tbody>
            {filteredErrors.length === 0 ? (
              ...
              filteredErrors.map((err) => (
                <tr key={err.id}>
                  <td>
                    <span className={'status-badge ' + (err.acked ? 'success' : 'danger')}>
                      {err.status}
                    </span>
                  </td>
                  <td>
                    <span className="status-badge info">{err.method}</span>
                  </td>
                  <td
                    style={{
