...
import {
  Bot, Power, Activity, Clock, Zap, RefreshCw,
    ...
    return (
      <div className="admin-page-container">
        <div className="page-header">
          <div>
            <h1 className="page-title"><Bot size={32} /> Bots Management</h1>
    ...
    <div className="admin-page-container">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            Monitor and control platform automation bots
          </p>
        </div>
      ...
      <div className="grid-3col">
        ...
        <div className="metric-card">
          ...
          <div className="metric-value" style={{ color: 'var(--accent-primary)' }}>{activeBots.length}</div>
        </div>
        <div className="metric-card">
          ...
          <div className="metric-value" style={{ color: 'var(--accent-danger)' }}>{inactiveBots.length}</div>
        </div>
      </div>
            ...
            return (
              <div key={bot.key || bot.id || i} className="metric-card">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginTop: '16px' }}>
                  <div>
                    <div style={{ fontSize: '11px', color: 'var(--text-faint)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '4px' }}>Status</div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <span className={'status-dot ' + (isActive ? 'online' : 'offline')}></span>
                      <span style={{ color: isActive ? 'var(--accent-primary)' : 'var(--accent-danger)', fontWeight: 600, fontSize: '13px' }}>
                        {isActive ? 'Running' : 'Stopped'}
                      </span>
                    </div>
                  </div>
                  <div>
                    <div style={{ fontSize: '11px', color: 'var(--text-faint)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '4px' }}>Last Run</div>
