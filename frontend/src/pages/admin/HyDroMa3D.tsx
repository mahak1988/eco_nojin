...
import {
  Play, Pause, FastForward, Sun, CloudRain, Wind, Thermometer,
  Sprout, Droplets, TrendingUp, Loader2, CheckCircle, XCircle,
  Coins, Users, MapPin, Settings, Bell, HelpCircle, Camera,
} from 'lucide-react';
import {
...
interface MotorResultBundle {
  ...
  result?: { status: string; summary?: Record<string, unknown>; outputs?: Record<string, unknown>; error_message?: string };
}
...
function cropColorFor(month: number): string {
  ...
  const ramp = ['#6b4c2a', '#7a5c30', '#8a7a35', '#5d8a2f', '#4a9a2a', '#3fae2a', '#35b52f', '#2fa829', '#5d9a35', '#8a8a35', '#9a7a3f', '#7a5c30'];
  return ramp[(month - 1 + 12) % 12];
}

function FieldPlot({
  position, size, color, selected, onClick, label,
}: {
    ...
    <group position={position} onClick={onClick}>
      ...
      {selected && label && (
        <Html center position={[0, 1.2, 0]} distanceFactor={14}>
          <div style={{
            background: 'rgba(20,25,30,0.92)', color: '#fff', padding: '6px 12px',
  ...
  return (
    <group position={position}>
      ...
      <Html center position={[0, 2.2, 0.9]} distanceFactor={12}>
        <div style={{
          background: 'rgba(20,25,30,0.85)', color: '#FFD700', padding: '2px 8px',
  ...
  return (
    <group position={position}>
      <mesh castShadow position={[0, 0.9, 0]}>
        ...
        <meshStandardMaterial color="#87CEEB" transparent opacity={0.55} metalness={0.2} roughness={0.1} />
      </mesh>
...
function Tree({ position, kind }: { position: [number, number, number]; kind: 'deciduous' | 'coniferous' }) {
  return (
    <group position={position}>
...
function Pond({ position, level }: { position: [number, number, number]; level: number }) {
  return (
    <mesh position={position} rotation={[-Math.PI / 2, 0, 0]}>
      ...
      <meshStandardMaterial color={level > 0.5 ? '#3b82c4' : '#6b7fa3'} transparent opacity={0.85} metalness={0.3} roughness={0.15} />
    </mesh>
  );
}
...
/* ------------------------------------------------------------------ scene */
function Scene({
  month, selectedPlot, onSelectPlot, precipLevel,
}: {
  month: number;
  ...
  const sunPos: [number, number, number] = [Math.cos(sunAngle) * 30, 22 + Math.sin(sunAngle) * 8, Math.sin(sunAngle) * 20 + 10];
  ...
  const trees: { pos: [number, number, number]; kind: 'deciduous' | 'coniferous' }[] = useMemo(() => {
  ...
  return (
    <>
      ...
      {plots.filter(p => p.kind === 'farm').map(p => (
        <group key={`rows-${p.i}`} position={[p.pos[0], 0.05, p.pos[2]]}>
          {[-1.6, -0.8, 0, 0.8, 1.6].map(off => (
          ...
          )).map((el, idx) => <group key={idx} position={[0, 0, off * 0.85]}>{el}</group>)}
        </group>
      ))}
      ...
      {/* trees */}
      {trees.map((t, i) => <Tree key={i} position={t.pos} kind={t.kind} />)}
...
function StatChip({ icon, value, color }: { icon: React.ReactNode; value: string; color?: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, fontWeight: 700, color: color || '#fff' }}>
      {icon} {value}
    </div>
  );
}
...
export default function HyDroMa3D() {
  ...
  /* data loading */
  useEffect(() => {
    api<{ sites: SiteRow[] }>('/manual/sites').then(d => {
      setSites(d.sites || []);
  ...
  useEffect(() => {
    if (!siteId) return;
    api<{ months: NormalRow[] }>(`/manual/climate-normals/${siteId}`).then(d => setNormals(d.months || [])).catch(() => {});
    api<{ crops: CropRow[] }>('/manual/crop-params').then(d => setCrops(d.crops || [])).catch(() => {});
  }, [siteId]);
  ...
  const chartData = normals.map(n => ({
    month: `${n.month}`, et0: n.et0_mm, precip: n.precip_mm, tavg: n.tavg_c,
  }));
  ...
  const runMotor = async (motor: string, extra: Record<string, unknown> = {}) => {
    ...
    try {
      ...
      if (pipeline?.setApiResult && data.result?.outputs) {
        pipeline.setApiResult(`motor:${motor}`, { outputs: data.result.outputs, summary: data.result.summary, site: data.site });
      }
    } catch (e: any) {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: 'calc(100vh - 130px)', minHeight: 560, borderRadius: 14, overflow: 'hidden', border: '1px solid var(--border-color)' }}>
      {/* 3D canvas */}
          ...
          >
            {sites.map(s => (
              <option key={s.site_id} value={s.site_id} style={{ background: '#16181f' }}>
                {s.site_id} — {s.admin1_city || s.province || s.country} ({s.lat?.toFixed(2)}, {s.lon?.toFixed(2)})
              </option>
            ))}
          </select>
          {selectedSite?.koppen && (
      ...
      {/* left rail */}
      <div style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', display: 'flex', flexDirection: 'column', gap: 10 }}>
        {[
      ...
      <div style={{ position: 'absolute', bottom: 12, left: 12, right: 12 }}>
        <Panel style={{ padding: '10px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 18, flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
            {currentNormal && (
              <>
                <StatChip icon={<Thermometer size={16} style={{ color: '#FF8A65' }} />} value={`${currentNormal.tavg_c.toFixed(1)}°C`} />
                <StatChip icon={<Sun size={16} style={{ color: '#FFD54F' }} />} value={`ET0 ${currentNormal.et0_mm.toFixed(1)}mm`} />
                <StatChip icon={<CloudRain size={16} style={{ color: '#64B5F6' }} />} value={`${currentNormal.precip_mm.toFixed(0)}mm`} />
                <StatChip icon={<Wind size={16} style={{ color: '#81C784' }} />} value={`${selectedSite?.koppen || '—'}`} />
              </>
            )}
          </div>
          <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {['$', '✂', '🔨', '🚜'].map((t, i) => (
                <div key={i} style={{
                  ...
                  background: 'rgba(255,255,255,0.08)', fontSize: 16, cursor: 'pointer',
            ...
            <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
              <button onClick={() => setPlaying(false)} style={{ background: 'none', border: 'none', color: playing ? '#B0BEC5' : '#fff', cursor: 'pointer' }}><Pause size={16} /></button>
              <button onClick={() => { setPlaying(true); setSpeed(1); }} style={{ background: 'none', border: 'none', color: playing && speed === 1 ? '#fff' : '#B0BEC5', cursor: 'pointer' }}><Play size={16} /></button>
              <button onClick={() => { setPlaying(true); setSpeed(3); }} style={{ background: 'none', border: 'none', color: playing && speed > 1 ? '#fff' : '#B0BEC5', cursor: 'pointer' }}><FastForward size={16} /></button>
              <span style={{ fontSize: 11, color: '#B0BEC5', fontFamily: 'monospace' }}>ماه {month}</span>
            </div>
          </div>
        </Panel>
      </div>
      ...
      <div style={{ position: 'absolute', top: 64, right: 12, bottom: 74, width: 350, display: 'flex', flexDirection: 'column', gap: 10, overflow: 'hidden' }}>
        <Panel style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div style={{ display: 'flex', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
            {([['plant', 'کاشت / AquaCrop'], ['irrigate', 'آبیاری'], ['analyze', 'تحلیل محصول']] as const).map(([k, label]) => (
              <button
                key={k}
                onClick={() => setPanelTab(k)}
                style={{
                  flex: 1, padding: '10px 6px', background: panelTab === k ? 'rgba(76,175,80,0.25)' : 'transparent',
                  border: 'none', borderBottom: panelTab === k ? '2px solid #4CAF50' : '2px solid transparent',
                  color: panelTab === k ? '#fff' : '#B0BEC5', fontSize: 12, fontWeight: 600, cursor: 'pointer',
                }}
              >{label}</button>
            ))}
          </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: 12, fontSize: 12.5 }}>
            {panelTab === 'plant' && (
              <>
                <div style={{ color: '#B0BEC5', marginBottom: 8 }}>محصولات (KC از دیتابیس دستی):</div>
                {crops.slice(0, 8).map(c => (
                  <div key={c.species_id} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 4px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                    <span>{c.scientific_name || c.species_id}</span>
                    <span style={{ fontFamily: 'monospace', color: '#81C784' }}>KC {c.kc_mid}</span>
                  </div>
                ))}
                <button
                  onClick={() => runMotor('aquacrop', { crop_name: 'wheat', planting_date: '2022-11-05', sim_start: '2022-11-01', sim_end: '2023-06-30' })}
                  disabled={running !== null}
                  style={{
                    width: '100%', marginTop: 10, padding: '10px', borderRadius: 10, border: 'none',
                    background: 'linear-gradient(135deg, #4CAF50, #388E3C)', color: '#fff', fontWeight: 700, cursor: 'pointer',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
                  }}
                >
                  {running === 'aquacrop' ? <Loader2 size={15} className="spin" /> : <Sprout size={15} />}
            ...
            {panelTab === 'irrigate' && (
              <>
                <div style={{ color: '#B0BEC5', marginBottom: 8 }}>ET0 ماهانه (نرمال اقلیمی سایت):</div>
                {normals.length > 0 && (
                  <ResponsiveContainer width="100%" height={160}>
                    <LineChart data={chartData}>
                      ...
                      <Tooltip contentStyle={{ background: '#16181f', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 8 }} />
                      <Legend />
                      <Line type="monotone" dataKey="et0" stroke="#FF8A65" name="ET0 (mm)" dot={false} />
                      <Line type="monotone" dataKey="precip" stroke="#64B5F6" name="بارش (mm)" dot={false} />
                    </LineChart>
                  </ResponsiveContainer>
                )}
                <button
                  ...
                  style={{
                    width: '100%', marginTop: 10, padding: '10px', borderRadius: 10, border: 'none',
                    background: 'linear-gradient(135deg, #29B6F6, #0288D1)', color: '#fff', fontWeight: 700, cursor: 'pointer',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
                  }}
                >
                  {running === 'irrigation' ? <Loader2 size={15} className="spin" /> : <Droplets size={15} />}
            ...
            {panelTab === 'analyze' && (
              <>
                <div style={{ color: '#B0BEC5', marginBottom: 8 }}>مشاور انتخاب محصول — ارزیابی بر اساس خاک/اقلیم سایت:</div>
                <button
                  onClick={() => runMotor('crop_advisor')}
                  disabled={running !== null}
                  style={{
                    width: '100%', padding: '10px', borderRadius: 10, border: 'none',
                    background: 'linear-gradient(135deg, #8b5cf6, #6366f1)', color: '#fff', fontWeight: 700, cursor: 'pointer',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
                  }}
                >
                  {running === 'crop_advisor' ? <Loader2 size={15} className="spin" /> : <TrendingUp size={15} />}
                  ...
                  <div style={{ marginTop: 12 }}>
                    ...
                    <ResponsiveContainer width="100%" height={150}>
                      <LineChart data={chartData}>
                        ...
                        <YAxis tick={{ fill: '#B0BEC5', fontSize: 10 }} />
                        <Tooltip contentStyle={{ background: '#16181f', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 8 }} />
                        <Line type="monotone" dataKey="tavg" stroke="#FFD54F" name="Tavg (°C)" dot={false} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                )}
        ...
        {running && (
          <Panel style={{ padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontSize: 12 }}>در حال اجرای {running} روی {siteId}… (CPU)</span>
          </Panel>
        )}
        {result && (
          <Panel style={{ padding: '12px 14px', maxHeight: 260, overflowY: 'auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
              ...
              <strong style={{ fontSize: 13 }}>{result.motor} — {result.result?.status}</strong>
            </div>
            {result.provenance && (
              <div style={{ fontSize: 11, color: '#B0BEC5', marginBottom: 8, direction: 'ltr', textAlign: 'left' }}>
                {JSON.stringify(result.provenance)}
              </div>
            )}
            {result.result?.summary && (
              <pre dir="ltr" style={{ fontSize: 11, whiteSpace: 'pre-wrap', background: 'rgba(255,255,255,0.05)', padding: 10, borderRadius: 8, margin: 0 }}>
                {JSON.stringify(result.result.summary, null, 2)}
              </pre>
            )}
            {result.result?.error_message && (
              <div style={{ color: '#FF8A80', fontSize: 12, marginTop: 6 }}>{result.result.error_message}</div>
            )}
          </Panel>
        )}
      </div>
