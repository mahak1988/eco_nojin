...
export const LmsCard: React.FC = () => {
  ...
  const mark = async (courseId: string, lessonId: string) => {
    ...
    if (token) {
      try {
        ...
        await fetch(
          `/api/v1/lms/progress?token=${encodeURIComponent(token)}&lesson_id=${encodeURIComponent(lessonId)}`,
          opts
        );
      } catch {
  ...
  return (
    <div className="card" style={{ padding: '1.1rem', marginTop: '1.5rem' }}>
      ...
      >
        ...
        {status === 'ok' && (
          <span style={{ fontSize: '0.72rem', color: 'var(--color-text-secondary)' }}>
            {courses.length} دوره رایگان ·{' '}
            {token ? (cloudReady ? '☁️ پیشرفت ابری' : 'در حال همگامسازی…') : 'پیشرفت محلی'}
          </span>
        )}
      </div>
      ...
      {status === 'error' && (
        <p style={{ fontSize: '0.82rem', color: '#ef4444' }}>⚠️ خطا در دریافت دورهها</p>
      )}
                        ...
                        >
                          <button
                            ...
                            style={{
                              ...
                              cursor: 'pointer',
                              padding: 0,
                              color: isDone ? '#10b981' : 'var(--color-text-secondary)',
                              display: 'flex',
                              marginTop: '0.1rem',
                            }}
      ...
      {status === 'ok' && (
        ...
        >
          محتوای آموزشی واقعی و رایگان · منبع:{' '}
          {source === 'supabase' ? 'دیتابیس ابری (Supabase)' : 'بکاند محلی'}.{' '}
          {token
            ? 'پیشرفت روی ابر ذخیره میشود (در همه دستگاهها).'
            : 'برای ذخیره ابری پیشرفت، وارد شوید.'}
        </p>
      )}
    </div>
  );
