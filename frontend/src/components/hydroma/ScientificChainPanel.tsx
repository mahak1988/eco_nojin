  ...
  const metrics: Metric[] = [
    {
      ...
      label: 'فرسایش (RUSLE)',
      value:
        erosion?.soil_loss_ton_ha_yr != null
          ? `${erosion.soil_loss_ton_ha_yr.toFixed(2)} t/ha/yr`
          : '—',
    ...
    },
    {
      ...
      value:
        (rothc.summary.soc_final_t_ha as number | undefined) != null
          ? `${Number(rothc.summary.soc_final_t_ha).toFixed(1)} t/ha`
          : '—',
      hint:
        (rothc.summary.soc_change_t_ha_yr as number | undefined) != null
          ? `تغییر: ${Number(rothc.summary.soc_change_t_ha_yr).toFixed(3)} t/ha/yr`
          : undefined,
    },
    {
      ...
      value:
        (aquacrop.summary.yield_ton_ha as number | undefined) != null
          ? `${Number(aquacrop.summary.yield_ton_ha).toFixed(2)} t/ha`
          : '—',
      hint:
        aquacrop.status === 'ok'
          ? `برداشت: ${String(aquacrop.summary.harvest_date ?? '—')}`
          : undefined,
    },
    {
      ...
      label: 'تخصیص آب (Pywr)',
      value:
        (water.summary.supply_reliability_pct as number | undefined) != null
          ? `${Number(water.summary.supply_reliability_pct).toFixed(1)}٪`
          : '—',
      hint:
        (water.summary.total_deficit_mcm as number | undefined) != null
          ? `کسری: ${Number(water.summary.total_deficit_mcm).toFixed(2)} MCM`
          : undefined,
    },
    {
      ...
      label: 'سیلاب (HEC-RAS)',
      value:
        (flood.summary.wse_m as number | undefined) != null
          ? `${Number(flood.summary.wse_m).toFixed(2)} m`
          : '—',
    ...
    },
    {
      ...
      value:
        (opt.summary.pareto_size as number | undefined) != null
          ? `${Number(opt.summary.pareto_size)} راه‌حل`
          : '—',
      hint:
        (opt.summary.best_yield_t_ha as number | undefined) != null
          ? `بهترین عملکرد: ${Number(opt.summary.best_yield_t_ha).toFixed(2)} t/ha`
          : undefined,
    },
  ];
  ...
  const chips = [
    {
      name: 'SWAT+',
      status: swat.status ?? '—',
      note: swat.status === 'prep_ready' ? 'آماده، نیاز به باینری' : undefined,
    },
    ...
    <div className="card" style={{ padding: '1.5rem' }}>
      ...
      {loading && (
        <p style={{ color: 'var(--color-text-secondary)', fontSize: '0.9rem' }}>
          اجرای ۶ موتور علمی — معمولاً ۳ تا ۸ ثانیه (اولین بار)…
        </p>
      )}
      ...
      {result && (
        <>
          {result.cache_hit && (
            ...
            >
              <CheckCircle2 size={14} /> نتیجه از کش بازیابی شد (شناسه:{' '}
              {result.chain_id.slice(0, 8)})
            </div>
          )}
      ...
      {!result && !loading && (
        <p style={{ color: 'var(--color-text-secondary)', fontSize: '0.9rem', margin: 0 }}>
          RUSLE ← SWAT+ ← Pywr ← RothC ← AquaCrop ← HEC-RAS ← NSGA-II — همه با داده واقعی اقلیم و
          خاک. وضعیت‌های «نیاز به باینری/اعتبارنامه» صادقانه نمایش داده می‌شوند.
        </p>
      )}
