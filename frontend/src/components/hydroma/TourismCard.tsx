  ...
  return (
    <div className="card" style={{ padding: '1.1rem', marginTop: '1.5rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '0.7rem' }}>
        ...
        {data?.status && (
          <span style={{ fontSize: '0.68rem', fontWeight: 800, padding: '0.22rem 0.6rem', borderRadius: 999, background: data.status === 'requires_setup' ? '#fef3c7' : '#ecfdf5', color: data.status === 'requires_setup' ? '#b45309' : '#065f46' }}>
      ...
      {data && (
        <>
          <ul style={{ margin: 0, padding: '0 1rem', fontSize: '0.76rem', color: 'var(--color-text)', lineHeight: 1.9 }}>
            {(data.capabilities ?? []).map((c) => (
              <li key={c}>{c}</li>
            ))}
          </ul>
          {data.note && (
            <p style={{ fontSize: '0.68rem', color: 'var(--color-text-secondary)', margin: '0.5rem 0 0', display: 'flex', alignItems: 'flex-start', gap: '0.3rem' }}>
