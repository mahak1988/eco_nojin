    ...
    <div className="card" style={{ padding: '1.1rem', marginTop: '1.5rem' }}>
      ...
      >
        ...
        <input
          ...
          step="0.01"
          value={refNdvi}
          onChange={(e) => setRefNdvi(parseFloat(e.target.value))}
          placeholder="reference NDVI"
          style={{
            padding: '0.32rem 0.5rem',
            borderRadius: 8,
            border: '1px solid var(--color-border)',
      ...
      <textarea
        ...
        rows={2}
        placeholder="سری NDVI فصل (با کاما)"
        style={{
          width: '100%',
          padding: '0.32rem 0.5rem',
          borderRadius: 8,
      ...
      {res && (
        ...
        >
          <div style={{ display: 'flex', gap: '0.6rem', flexWrap: 'wrap', fontWeight: 700 }}>
            ...
            <span style={{ color: res.trigger_active ? '#b45309' : '#0d9488' }}>
              {res.trigger_active ? '⚠️ فعال (غیرعادی)' : 'عادی'}
            </span>
          ...
          {res.note && (
            ...
            >
              <TrendingDown size={10} style={{ verticalAlign: -2 }} /> {res.note}
            </p>
          )}
        </div>
      )}
