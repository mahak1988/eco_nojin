...
interface DroughtData {
  ...
  latest?: {
    month?: string;
    spi?: number | null;
    spi_class?: { label?: string; level?: string };
    spei?: number | null;
  };
...
export const DroughtCard: React.FC = () => {
  ...
  const chart = useMemo(() => {
    ...
    return {
      ...
      xAxis: {
        type: 'category',
        data: s.map((p) => p.month ?? ''),
        axisLabel: { fontSize: 9, color: '#94a3b8', interval: 11 },
      },
      yAxis: {
        type: 'value',
      ...
      {status === 'ok' && data && (
        <>
          ...
          >
            ...
            >
              آخرین ماه: <strong>{data.latest?.month}</strong> · SPI {data.latest?.spi} / SPEI{' '}
              {data.latest?.spei}
            </span>
            <span
              style={{
          ...
          {data.alert && (
            ...
            >
              <AlertTriangle size={11} /> هشدار اولیه: کانال داشبورد فعال؛ اتصال USSD/SMS پس از
              فراهم‌شدن درگاه پیامک انجام می‌شود.
            </p>
          )}
          {data.note && (
            <p
