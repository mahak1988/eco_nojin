...
/* ---------- کوچک‌ها ---------- */
const Field: React.FC<{ label: string; value: React.ReactNode; strong?: boolean }> = ({
  label,
  value,
  strong,
}) => (
  ...
  >
    ...
    <div style={{ fontSize: strong ? '0.95rem' : '0.85rem', fontWeight: strong ? 800 : 600 }}>
      {value}
    </div>
  </div>
);
...
const ModuleCard: React.FC<{
  ...
  <div
    className="card"
    style={{ padding: '1.1rem', display: 'flex', flexDirection: 'column', gap: '0.85rem' }}
  >
...
export const HydromaModules: React.FC<HydromaModulesProps> = ({ realLand, chain, onView3D }) => {
  ...
  const pareto =
    (opt?.outputs?.pareto_front as
      Array<{ erosion_t_ha_yr: number; yield_ton_ha: number; deficit_mcm: number }> | undefined) ??
    [];
    ...
    >
      ...
      >
        ...
        >
          <Field
            label="بارش سالانه"
            value={`${num(climate?.annual_rainfall_mm)?.toFixed(1) ?? '—'} mm`}
            strong
          />
          <Field
            label="دمای میانگین"
            value={`${num(climate?.avg_temp_c)?.toFixed(1) ?? '—'} °C`}
            strong
          />
          <Field
            label="ET0 سالانه"
            value={`${num(climate?.annual_et0_mm)?.toFixed(0) ?? '—'} mm`}
          />
          ...
          <Field
            label="آخرین روز"
            value={climate?.latest?.date ? String(climate.latest.date).slice(5) : '—'}
          />
        </div>
      </ModuleCard>
      ...
      >
        ...
        >
          ...
          <Field
            label="شن/سیلت/رس"
            value={`${num(soil?.sand_pct)?.toFixed(0) ?? '—'}/${num(soil?.silt_pct)?.toFixed(0) ?? '—'}/${num(soil?.clay_pct)?.toFixed(0) ?? '—'}٪`}
          />
          <Field
            label="SOC"
            value={soil?.soc_pct != null ? `${soil.soc_pct.toFixed(2)}٪` : '—'}
            strong
          />
          ...
          <Field
            label="جرم مخصوص"
            value={`${num(soil?.bulk_density_g_cm3)?.toFixed(2) ?? '—'} g/cm³`}
          />
      ...
      >
        ...
        >
          <Field
            label="هدررفت خاک"
            value={`${num(erosion?.soil_loss_ton_ha_yr)?.toFixed(2) ?? '—'} t/ha/yr`}
            strong
          />
      ...
      >
        <ReactECharts
          ...
          option={{
            ...
            xAxis: {
              type: 'category',
              data: socSeries.map((_, i) => `سال ${i + 1}`),
              axisLabel: { fontSize: 9, interval: Math.max(0, Math.floor(socSeries.length / 8)) },
            },
        ...
        >
          <Field
            label="SOC اولیه"
            value={`${num(rothc?.outputs?.initial_soc_t_ha)?.toFixed(1) ?? '—'} t/ha`}
          />
          <Field
            label="SOC نهایی"
            value={`${num(rothc?.summary?.soc_final_t_ha)?.toFixed(1) ?? '—'} t/ha`}
            strong
          />
          <Field
            label="تغییر سالانه"
            value={`${num(rothc?.summary?.soc_change_t_ha_yr)?.toFixed(3) ?? '—'} t/ha/yr`}
          />
      ...
      >
            ...
            series: [
              {
                ...
                data: [
                  {
                    value: num(aquacrop?.summary?.yield_ton_ha) ?? 0,
                    itemStyle: { color: '#eab308' },
                  },
                  {
                    value: num(aquacrop?.outputs?.biomass_ton_ha) ?? 0,
        ...
        >
          <Field
            label="عملکرد"
            value={`${num(aquacrop?.summary?.yield_ton_ha)?.toFixed(2) ?? '—'} t/ha`}
            strong
          />
          <Field
            label="زی‌توده"
            value={`${num(aquacrop?.outputs?.biomass_ton_ha)?.toFixed(2) ?? '—'} t/ha`}
          />
          <Field
            label="آبیاری"
            value={`${num(aquacrop?.summary?.irrigation_mm)?.toFixed(0) ?? '—'} mm`}
          />
          <Field
            label="بهره‌وری آب"
            value={`${num(aquacrop?.outputs?.water_productivity_kg_m3)?.toFixed(2) ?? '—'} kg/m³`}
          />
      ...
      >
        <ReactECharts
          ...
          option={{
            ...
            xAxis: {
              type: 'category',
              data: supplySeries.map((_, i) => `ماه ${i + 1}`),
        ...
        >
          <Field
            ...
            value={`${num(water?.summary?.supply_reliability_pct)?.toFixed(1) ?? '—'}٪`}
            strong
          />
          <Field
            label="کسری کل"
            value={`${num(water?.summary?.total_deficit_mcm)?.toFixed(2) ?? '—'} MCM`}
          />
          <Field
            label="تقاضای کل"
            value={`${num(water?.outputs?.total_demand_mcm)?.toFixed(2) ?? '—'} MCM`}
          />
          <Field
            label="تأمین کل"
            value={`${num(water?.outputs?.total_supply_mcm)?.toFixed(2) ?? '—'} MCM`}
          />
          <Field label="شبکه" value={String(water?.outputs?.network ?? '—')} />
          <Field
            label="رواناب ورودی"
            value={`${num(inputs?.annual_runoff_mcm)?.toFixed(3) ?? '—'} MCM`}
          />
        </div>
      </ModuleCard>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', flexWrap: 'wrap' }}>
          <Field
            label="ارتفاع آب (Manning)"
            value={`${num(flood?.summary?.wse_m)?.toFixed(2) ?? '—'} m`}
            strong
          />
        ...
        <p style={{ fontSize: '0.78rem', color: 'var(--color-text-secondary)', margin: 0 }}>
          {flood?.summary?.requires_hecras_install === true ? (
            <>
              باینری رایگان HEC-RAS نصب نیست — با نصب آن، عمق و پهنه سیلاب واقعی (و رندر deck.gl)
              فعال می‌شود:{' '}
              <a
      ...
      >
          ...
          option={{
            ...
            tooltip: {
              ...
              formatter: (p: { data: [number, number, number] }) =>
                `فرسایش ${p.data[0].toFixed(2)} · عملکرد ${p.data[1].toFixed(2)} · کسری ${p.data[2].toFixed(2)}`,
            },
            xAxis: {
              type: 'value',
              name: 'فرسایش t/ha/yr',
            ...
            series: [
              {
                ...
                data: pareto.map(
                  (p) =>
                    [p.erosion_t_ha_yr, p.yield_ton_ha, p.deficit_mcm] as [number, number, number]
                ),
        ...
        >
          <Field
            label="راه‌حل‌های پارتو"
            value={String(opt?.outputs?.pareto_size ?? opt?.summary?.pareto_size ?? '—')}
            strong
          />
          <Field
            label="بهترین عملکرد"
            value={`${num(opt?.summary?.best_yield_t_ha)?.toFixed(2) ?? '—'} t/ha`}
          />
          <Field
            label="کمترین فرسایش"
            value={`${num(opt?.summary?.min_erosion_t_ha_yr)?.toFixed(2) ?? '—'} t/ha/yr`}
          />
      ...
      >
        ...
        >
          ...
          <Field
            label="SOC اولیه"
            value={`${num(inputs?.soc_initial_t_ha)?.toFixed(1) ?? '—'} t/ha`}
          />
        ...
        <p style={{ fontSize: '0.78rem', color: 'var(--color-text-secondary)', margin: 0 }}>
          {swat?.summary?.run_requires_executable === true && (
            <>
              اجرای کامل نیازمند باینری رایگان:{' '}
              <a href="https://swat.tamu.edu/software/plus/" target="_blank" rel="noreferrer">
                دانلود SWAT+ از Texas A&amp;M
              </a>
            </>
          )}
        </p>
      ...
      >
        ...
        >
          ...
          <Field
            label="ابر"
            value={sat?.cloud_cover != null ? `${sat.cloud_cover.toFixed(0)}٪` : '—'}
          />
        </div>
        <p style={{ fontSize: '0.78rem', color: 'var(--color-text-secondary)', margin: 0 }}>
          {sat?.status === 'credentials_required' && (
            <>
              اعتبارنامه رایگان CDSE لازم است — با افزودن به .env، NDVI/LAI/C-factor واقعی
              Sentinel-2 فعال می‌شود.
            </>
          )}
        </p>
      </ModuleCard>
