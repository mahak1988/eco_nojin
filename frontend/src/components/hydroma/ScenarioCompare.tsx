...
export const ScenarioCompare: React.FC<ScenarioCompareProps> = ({ lat, lon, crop = 'wheat' }) => {
  ...
  const run = async () => {
    ...
    try {
      const [base, terrace] = await Promise.all([
        fetchScientificChain(lat, lon, {
          crop,
          plantingDate: '2024-11-15',
          slopePct: SCENARIOS[0].slopePct,
          catchmentKm2: 10,
          optimize: false,
        }),
        fetchScientificChain(lat, lon, {
  ...
  const metricOf = (
    r: ScientificChainResult | null,
    kind: 'erosion' | 'yield' | 'soc' | 'reliability'
  ): number | null => {
    if (!r) return null;
    switch (kind) {
      case 'erosion':
        return r.erosion?.soil_loss_ton_ha_yr ?? null;
      case 'yield':
        return (r.aquacrop?.summary?.yield_ton_ha as number | undefined) ?? null;
      case 'soc':
        return (r.rothc?.summary?.soc_final_t_ha as number | undefined) ?? null;
      case 'reliability':
        return (r.water?.summary?.supply_reliability_pct as number | undefined) ?? null;
    }
  };
  ...
  return (
    <div className="card" style={{ padding: '1.25rem' }}>
      ...
      {error && (
        <p style={{ color: '#ef4444', fontSize: '0.82rem', margin: '0 0 0.6rem' }}>{error}</p>
      )}
      ...
      {!results.baseline && !loading && !error && (
        <p style={{ color: 'var(--color-text-secondary)', fontSize: '0.82rem', margin: 0 }}>
          دو اجرای واقعی زنجیره (پایه در برابر تراسبندی) — تفاوت فقط در پارامتر شیب؛ بقیه داده‌ها
          یکسان و واقعی است.
        </p>
      )}
    </div>
