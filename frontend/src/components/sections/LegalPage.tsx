    ...
    <>
      <section className="px-4 pb-4 pt-14 sm:px-6 lg:pt-20">
        <Reveal className="mx-auto flex max-w-3xl flex-col items-start gap-4">
          ...
          <h1 className="text-3xl font-extrabold leading-snug text-[var(--color-night-100)] sm:text-5xl">
            {content.title}
          </h1>
          <p className="max-w-2xl text-base leading-8 text-[var(--color-night-200)]/60">{content.lead}</p>
          ...
          <p className="w-full rounded-2xl border border-[var(--color-sand-500)]/25 bg-[var(--color-sand-500)]/8 p-4 text-xs leading-6 text-[var(--color-sand-200)]/90">
            {content.draftNote}
          </p>
        </Reveal>
      </section>
      ...
      <section className="px-4 py-10 sm:px-6">
        <div className="mx-auto flex max-w-3xl flex-col gap-5">
          {content.sections.map((section, index) => (
            <Reveal key={section.title} delay={Math.min(index * 0.04, 0.2)}>
              <article className="glass rounded-3xl p-7">
                <h2 className="flex items-baseline gap-2 text-lg font-extrabold text-[var(--color-night-100)]">
                  <span className="text-[var(--color-leaf-400)]/70" aria-hidden>
                ...
                <div className="mt-3 flex flex-col gap-2.5">
                  {section.body.map((paragraph) => (
                    <p key={paragraph} className="text-sm leading-8 text-[var(--color-night-200)]/70">
                      {paragraph}
                    </p>
                  ))}
                </div>
          ...
          <Reveal>
            ...
            >
              <span className="font-bold text-[var(--color-night-200)]/50">{t.footer.legalTitle}:</span>
              {siblings.map((sib) => (
                <Link
                  key={sib.to}
                  to={sib.to}
                  className="rounded-full bg-white/5 px-3 py-1.5 font-bold text-[var(--color-leaf-300)] transition-colors hover:bg-[var(--color-leaf-500)]/15"
                >
                  {sib.label}
                </Link>
              ))}
