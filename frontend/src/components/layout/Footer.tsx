...
const accents: Accent[] = [
  ...
  { text: 'text-iris-300', softBg: 'bg-iris-400/10', border: 'border-iris-400/25', dot: 'bg-iris-300', hoverText: 'hover:text-iris-300' },
];
      ...
      <div className="mx-auto max-w-6xl px-4 pt-14 sm:px-6">
        ...
        <div className="glass mb-6 grid gap-8 rounded-[2rem] p-7 sm:p-9 lg:grid-cols-[1.35fr_1fr] lg:items-center">
          <div className="flex flex-col gap-4">
            ...
            <p className="max-w-md text-sm leading-7 text-ink-2">{t.footer.desc}</p>
            <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
              <a
                href={`mailto:${t.about.email}`}
                className="inline-flex items-center gap-2 text-sm text-ink-2 transition-colors hover:text-leaf-300"
                dir="ltr"
              >
                <Mail className="h-4 w-4 shrink-0 text-leaf-400/80" aria-hidden />
                {t.about.email}
              </a>
              <span className="inline-flex items-center gap-2 text-sm text-ink-2" dir="ltr">
          ...
          >
            ...
            <p className="mt-1 text-[11px] leading-5 text-ink-3">{newsLabels.hint}</p>
            <div className="mt-4 flex gap-2">
              <input
                ...
                className="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-xs text-ink-1 placeholder:text-ink-3 focus:border-aqua-400/60 focus:outline-none"
                required
              />
              <button
                type="submit"
                ...
                >
                  {group.links.map((link) => (
                    <Link
                      key={link.to}
                      to={link.to}
                      className={`w-fit rounded-lg px-2 py-1 text-[13px] text-ink-2 transition-colors hover:bg-white/5 ${accent.hoverText}`}
                    >
                      {link.label}
                    </Link>
                  ))}
          ...
          <nav aria-label={legal.title} className="flex flex-wrap items-center justify-center gap-x-1 gap-y-1">
            {legal.links.map((link) => (
              <Link
                key={link.to}
                to={link.to}
                className="rounded-full px-3.5 py-1.5 text-xs text-ink-2 transition-colors hover:bg-white/5 hover:text-leaf-300"
              >
                {link.label}
              </Link>
            ))}
          </nav>
          <p className="text-center text-xs text-ink-3">{t.footer.legal}</p>
        </div>
      </div>
    </footer>
  );
