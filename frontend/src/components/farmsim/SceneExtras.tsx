...
export function samplePlotData(t: TerrainData, x: number, z: number): PlotData {
  ...
  const slope =
    (Math.atan(Math.sqrt(((zR - zL) / 2) ** 2 + ((zD - zU) / 2) ** 2) / 20) * 180) / Math.PI;
  return {
    moisture: t.moisture[gz]?.[gx] ?? 0,
    ndvi: Math.max(
      0,
      Math.min(1, (t.moisture[gz]?.[gx] ?? 0) * 0.6 + ((elev - t.minElevation) / range) * 0.4)
    ),
      ...
      >
        <div
          style={{
            ...
            borderRadius: 10,
            padding: '8px 12px',
            fontFamily: 'var(--font-persian, Tahoma)',
            fontSize: 11,
        ...
        >
          ...
          <div>
            💧 رطوبت: <b>{Math.round(dta.moisture * 100)}%</b>
          </div>
          <div>
            🌿 NDVI: <b>{dta.ndvi.toFixed(2)}</b>
          </div>
          <div>
            ⛰️ شیب: <b>{dta.slope.toFixed(1)}°</b>
          </div>
          <div>
            ⚠️ فرسایش: <b>{dta.erosion.toFixed(1)}</b>
          </div>
          <div>
            📐 ارتفاع: <b>{Math.round(dta.elevation)}m</b>
          </div>
        </div>
      </Html>
    </group>
...
export function Crops({
  ...
  const plants = useMemo(() => {
    ...
    const rows = Math.floor(d / 0.7),
      cols = Math.floor(w / 0.5);
  ...
  return (
    <group>
      <Instances limit={pines.length} castShadow>
        ...
        {pines.map((t, i) => (
          <Instance key={i} position={[t.x, t.y + 1.6 * t.s, t.z]} scale={t.s} />
        ))}
      </Instances>
      <Instances limit={decid.length} castShadow>
        ...
        {decid.map((t, i) => (
          <Instance key={i} position={[t.x, t.y + 1.8 * t.s, t.z]} scale={t.s} />
        ))}
      </Instances>
    </group>
  );
  ...
  return (
    <group position={[position[0], y, position[2]]} rotation={[0, rotation, 0]} scale={scale}>
      <mesh position={[0, 1.2, 0]} castShadow>
        <boxGeometry args={[4, 2.4, 6]} />
        <meshStandardMaterial color="#7a5a42" roughness={0.9} />
      </mesh>
  ...
  return (
    <group position={[position[0], y, position[2]]} scale={scale}>
      <mesh position={[0, 2.2, 0]} castShadow>
        <cylinderGeometry args={[1.1, 1.1, 4.4, 20]} />
        <meshStandardMaterial color="#9aa0a8" metalness={0.7} roughness={0.3} />
      </mesh>
      <mesh position={[0, 4.6, 0]} castShadow>
        <coneGeometry args={[1.1, 0.9, 20]} />
        <meshStandardMaterial color="#7a8088" metalness={0.7} roughness={0.3} />
      </mesh>
    </group>
  );
}
