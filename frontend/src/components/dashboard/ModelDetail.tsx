  ...
  if (!entry) {
    return (
      <div className="glass rounded-3xl p-8 text-center">
        ...
        <Link to="/dashboard" className="mt-3 inline-block text-xs font-bold text-leaf-300 hover:underline">
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Reveal className="flex flex-col gap-3">
        <Link
          to="/dashboard"
          className="inline-flex w-fit items-center gap-2 text-xs font-bold text-leaf-300 hover:text-leaf-200"
        >
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="rounded-full bg-aqua-500/12 px-3 py-1 text-[11px] font-bold text-aqua-300">
            {isFa ? (category?.nameFa ?? entry.category) : (category?.nameEn ?? entry.category)}
          </span>
          <span
            className={`rounded-full px-3 py-1 text-[11px] font-bold ${
              entry.impl.includes('calculator') ? 'bg-violet-400/15 text-violet-300' : 'bg-white/8 text-emerald-100/55'
            }`}
          >
            {implLabel}
          </span>
        </div>
        <h1 className="text-2xl font-extrabold leading-snug text-emerald-50 sm:text-3xl" dir="ltr">
          {isFa ? entry.nameFa : entry.nameEn}
        </h1>
        <p className="max-w-3xl text-sm leading-8 text-emerald-100/65">
          {isFa ? entry.descFa : entry.descEn}
        </p>
        <p className="text-xs text-emerald-100/40" dir="ltr">
      ...
      {calcKey ? (
        ...
        <Reveal>
          <div className="glass flex items-start gap-3 rounded-2xl p-5">
            <FlaskConical className="mt-0.5 h-5 w-5 shrink-0 text-aqua-300" aria-hidden />
      ...
      <Reveal>
        <div className="glass flex flex-wrap items-center gap-2 rounded-2xl p-4 text-xs">
          <ScrollText className="h-4 w-4 text-leaf-400/80" aria-hidden />
          <span className="font-bold text-emerald-100/60">{isFa ? 'پیوندهای مرتبط:' : 'Related:'}</span>
          <Link to="/transparency" className="rounded-full bg-white/5 px-3 py-1 font-bold text-emerald-100/70 hover:text-leaf-300">
            {t.footer.quickLinks.transparency}
          </Link>
          <Link to="/terms" className="rounded-full bg-white/5 px-3 py-1 font-bold text-emerald-100/70 hover:text-leaf-300">
            {t.footer.legalLinks.terms}
          </Link>
          <Link to="/eco-coin" className="rounded-full bg-white/5 px-3 py-1 font-bold text-emerald-100/70 hover:text-leaf-300">
            Eco Coin
          </Link>
        </div>
      </Reveal>
