...
export interface PolygonArea {
  ...
  name: string; color: string; area?: number;
}
...
export const EngineeringOpsPanel = memo(function EngineeringOpsPanel({
  operations, polygons, onAddOperation, onMoveOperation, onDeleteOperation,
  onAddPolygon, onDeletePolygon, mode, onModeChange,
  currentDrawing, onDrawingUpdate, onFinishPolygon
}: Props) {
  ...
  return (
    <div style={{ width: '100%' }}>
      {/* Tool Mode Selector */}
      <div style={cardStyle}>
        <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.6)', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '6px' }}>
          ...
          <button onClick={() => onModeChange('draw-polygon')}
            style={{
              padding: '12px 8px', borderRadius: '10px',
        ...
        {mode === 'draw-polygon' && (
          <div style={{ marginTop: '8px', padding: '10px', background: 'rgba(245, 158, 11, 0.15)', borderRadius: '8px', fontSize: '11px', color: '#fbbf24' }}>
            💡 {isFa ? 'روی زمین کلیک کنید تا نقطه اضافه شود' : 'Click on terrain to add points'}
            <div style={{ display: 'flex', gap: '6px', marginTop: '8px' }}>
              <button onClick={onFinishPolygon} disabled={currentDrawing.length < 3}
                style={{
                ...
                ✓ {isFa ? `اتمام (${currentDrawing.length} نقطه)` : `Finish (${currentDrawing.length})`}
              </button>
              <button onClick={() => onDrawingUpdate([])}
                style={{
                  flex: 1, padding: '8px', borderRadius: '6px',
      ...
      <div style={cardStyle}>
        <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.6)', marginBottom: '8px', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: '6px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          {data.engineeringOps.map(op => (
            <button key={op.id}
              ...
              style={{
                padding: '12px', borderRadius: '10px', textAlign: 'start',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.5)' }}>
                    💰 ${op.cost.toLocaleString()} • <Navigation size={10} style={{ display: 'inline' }} /> {isFa ? 'کلیک برای افزودن' : 'Click to add'}
                  </div>
                </div>
              </div>
            </button>
      ...
      {operations.length > 0 && (
        <div style={cardStyle}>
          <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.6)', marginBottom: '8px', textTransform: 'uppercase', display: 'flex', justifyContent: 'space-between' }}>
            <span>{isFa ? `جانمایی شده (${operations.length})` : `Placed (${operations.length})`}</span>
            ...
            {operations.map(op => {
              ...
              return (
                <div key={op.id} style={{
                  ...
                  <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: '10px', fontFamily: 'monospace' }}>
                    [{op.x.toFixed(1)},{op.y.toFixed(1)}]
                  </span>
                  <button onClick={() => onDeleteOperation(op.id)}
                    style={{
      ...
      {polygons.length > 0 && (
        <div style={cardStyle}>
          <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.6)', marginBottom: '8px', textTransform: 'uppercase' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            {polygons.map(poly => (
              <div key={poly.id} style={{
                display: 'flex', alignItems: 'center', gap: '8px',
                padding: '8px', background: `${poly.color}15`,
                borderRadius: '8px', fontSize: '11px',
                ...
                <button onClick={() => onDeletePolygon(poly.id)}
                  style={{
                    padding: '4px 6px', borderRadius: '4px', border: 'none',
                    background: 'rgba(239, 68, 68, 0.3)', color: '#fca5a5',
                    cursor: 'pointer',
      ...
      <div style={{
        ...
        <div style={{ lineHeight: '1.6' }}>
          🖱️ <b>{isFa ? 'چرخش' : 'Rotate'}</b>: {isFa ? 'کلیک چپ + درگ' : 'Left click + drag'}<br/>
          🔍 <b>{isFa ? 'زوم' : 'Zoom'}</b>: {isFa ? 'اسکرول موس' : 'Mouse wheel'}<br/>
          ✋ <b>{isFa ? 'جابجایی' : 'Pan'}</b>: {isFa ? 'کلیک راست + درگ' : 'Right click + drag'}<br/>
          📐 <b>{isFa ? 'ترسیم' : 'Draw'}</b>: {isFa ? 'حالت ترسیم → کلیک روی زمین' : 'Draw mode → Click terrain'}<br/>
          📍 <b>{isFa ? 'جانمایی' : 'Place'}</b>: {isFa ? 'انتخاب عملیات → کلیک' : 'Select op → Click'}
        </div>
      </div>
    </div>
  );
...
export function computePolygonArea(points: Array<{x: number; y: number}>, cellSize = 20/64): number {
