...
const FloodZoneMap = React.lazy(() =>
  import('./FloodZoneMap').then((m) => ({ default: m.FloodZoneMap }))
);
    ...
    >
      <span
        style={{
          ...
          height: 7,
          borderRadius: '50%',
          background: fail ? '#ef4444' : ok ? '#10b981' : '#f59e0b',
        }}
      />
      {status ?? '—'}
    </span>
  ...
  const tabs = useMemo(
    () => [
          ...
          <div>
            <div className="sci-grid">
              <Card title="فرسایش (RUSLE)" icon={<Mountain size={18} />}>
                <div className="sci-bignum">
                  {erosion?.soil_loss_ton_ha_yr?.toFixed(2) ?? '—'}{' '}
                  <small style={{ fontSize: '0.8rem', fontWeight: 400 }}>t/ha/yr</small>
                </div>
              ...
              <Card title="کربن خاک (RothC)" icon={<Leaf size={18} />}>
                <div className="sci-bignum">
                  {num(rothc?.summary?.soc_final_t_ha)?.toFixed(1) ?? '—'}{' '}
                  <small style={{ fontSize: '0.8rem', fontWeight: 400 }}>t/ha</small>
                </div>
                <div style={{ color: 'var(--color-text-secondary)', fontSize: '0.8rem' }}>
                  تغییر: {num(rothc?.summary?.soc_change_t_ha_yr)?.toFixed(3) ?? '—'} t/ha/yr
                </div>
              </Card>
              <Card title="عملکرد گندم (AquaCrop)" icon={<Sprout size={18} />}>
                <div className="sci-bignum">
                  {num(aquacrop?.summary?.yield_ton_ha)?.toFixed(2) ?? '—'}{' '}
                  <small style={{ fontSize: '0.8rem', fontWeight: 400 }}>t/ha</small>
                </div>
                <div style={{ color: 'var(--color-text-secondary)', fontSize: '0.8rem' }}>
                  آبیاری: {num(aquacrop?.summary?.irrigation_mm)?.toFixed(0) ?? '—'} mm
                </div>
              </Card>
              <Card title="تخصیص آب (Pywr)" icon={<Waves size={18} />}>
                <div className="sci-bignum">
                  {num(water?.summary?.supply_reliability_pct)?.toFixed(1) ?? '—'}{' '}
                  <small style={{ fontSize: '0.8rem', fontWeight: 400 }}>٪</small>
                </div>
                <div style={{ color: 'var(--color-text-secondary)', fontSize: '0.8rem' }}>
                  کسری: {num(water?.summary?.total_deficit_mcm)?.toFixed(2) ?? '—'} MCM
                </div>
              </Card>
            </div>
            {r?.calibration?.status === 'no_observed_data' && (
              <p style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)' }}>
                📊 کالیبراسیون: {String(r.calibration.status)} — پس از فراهم‌شدن سری مشاهداتی، KGE
                محاسبه می‌شود (هدف ≥ ۰.۵۵).
              </p>
            )}
          </div>
        ),
        ...
        content: (
          <Card title="فرسایش خاک — RUSLE (واقعی)" icon={<Mountain size={18} />}>
            ...
            <p style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)', margin: 0 }}>
              فرمول جهانی هدررفت خاک (USLE/RUSLE) با ورودی‌های واقعی: باران (ERA5)، ضریب K از بافت
              خاک SoilGrids، شیب از DEM.
            </p>
          </Card>
        ),
      },
      {
        ...
        content: (
          <Card title="کربن آلی خاک — RothC-26.3 (pyRothC واقعی)" icon={<Leaf size={18} />}>
            <div style={{ display: 'flex', gap: '2rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
              <div>
                <div style={{ fontSize: '2rem', fontWeight: 800 }}>
                  {num(rothc?.summary?.soc_final_t_ha)?.toFixed(1) ?? '—'}{' '}
                  <small style={{ fontSize: '0.9rem', fontWeight: 400 }}>t C/ha</small>
                </div>
                <div style={{ color: 'var(--color-text-secondary)', fontSize: '0.8rem' }}>
                  اولیه: {num(rothc?.outputs?.initial_soc_t_ha)?.toFixed(1) ?? '—'} → نهایی:{' '}
                  {num(rothc?.summary?.soc_final_t_ha)?.toFixed(1) ?? '—'} (
                  {num(rothc?.summary?.soc_change_t_ha_yr)?.toFixed(3) ?? '—'} t/ha/yr)
                </div>
              </div>
              {rothcPools && (
            ...
            >
              موتور: {String(rothc?.outputs?.engine ?? 'pyRothC')} · استخرهای DPM/RPM/BIO/HUM/IOM ·
              ورودی اقلیم/رس واقعی.
            </p>
          </Card>
        ),
      },
          ...
          <Card title="عملکرد محصول — AquaCrop-OSPy (واقعی)" icon={<Sprout size={18} />}>
            ...
            >
              {[
                ...
                [
                  'بهره‌وری آب',
                  `${num(aquacrop?.outputs?.water_productivity_kg_m3)?.toFixed(2) ?? '—'} kg/m³`,
                ],
            ...
            <p style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)', margin: 0 }}>
              محصول: {state.crop} · تاریخ کاشت ۲۰۲۴-۱۱-۱۵ · آب‌وهوای روزانه واقعی (ERA5) + بافت خاک
              واقعی (SoilGrids).
            </p>
          </Card>
        ),
          ...
          <Card title="تخصیص آب حوضه — Pywr (واقعی)" icon={<Waves size={18} />}>
            <div style={{ display: 'flex', gap: '2rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
              <div>
                <div style={{ fontSize: '2rem', fontWeight: 800 }}>
                  {num(water?.summary?.supply_reliability_pct)?.toFixed(1) ?? '—'}{' '}
                  <small style={{ fontSize: '0.9rem', fontWeight: 400 }}>٪ قابلیت اطمینان</small>
                </div>
                <div style={{ color: 'var(--color-text-secondary)', fontSize: '0.8rem' }}>
                  کسری کل: {num(water?.summary?.total_deficit_mcm)?.toFixed(2) ?? '—'} MCM
                </div>
              </div>
            </div>
              ...
              <div style={{ marginBottom: '0.75rem' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-end', gap: 2, height: 70 }}>
                  {supplySeries.map((v, i) => (
                    <div
                      key={i}
                      title={`ماه ${i + 1}: ${v.toFixed(3)}`}
                      style={{
                        flex: 1,
                        height: `${Math.min(100, (v / Math.max(1e-6, Math.max(...supplySeries))) * 100)}%`,
                        background: '#3b82f6',
                        borderRadius: '2px 2px 0 0',
                        minHeight: 2,
                      }}
                ...
                <div style={{ display: 'flex', alignItems: 'flex-end', gap: 2, height: 70 }}>
                  {storageSeries.map((v, i) => (
                    <div
                      ...
                      style={{
                        flex: 1,
                        height: `${Math.min(100, (v / Math.max(1e-6, Math.max(...storageSeries))) * 100)}%`,
                        background: '#06b6d4',
                        borderRadius: '2px 2px 0 0',
                        minHeight: 2,
                      }}
            ...
            >
              شبکه: ورودی (رواناب) ← مخزن ← تقاضای کشاورزی + نیاز محیط‌زیست · گام ماهانه · موتور:
              pywr 1.31.
            </p>
          </Card>
        ),
          ...
          <Card title="سیلاب — HEC-RAS (خودکارسازی) + نقشه deck.gl" icon={<FloodIcon size={18} />}>
            ...
            >
              ...
              <span style={{ fontSize: '0.85rem' }}>
                ارتفاع آب (تقریبی Manning):{' '}
                <strong>{num(flood?.summary?.wse_m)?.toFixed(2) ?? '—'} m</strong>
              </span>
          ...
          <Card title="بهینه‌سازی چندهدفه — NSGA-II (pymoo)" icon={<Cpu size={18} />}>
            ...
            >
              {[
                ...
                [
                  'کمترین فرسایش',
                  `${num(opt?.summary?.min_erosion_t_ha_yr)?.toFixed(2) ?? '—'} t/ha/yr`,
                ],
              ].map(([k, v]) => (
                <div key={k} className="sci-metric">
                  <div style={{ fontSize: '0.72rem', color: 'var(--color-text-secondary)' }}>
            ...
            <p style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)', margin: 0 }}>
              ۴ هدف: کاهش فرسایش، کاهش کسری آب، افزایش عملکرد، افزایش SOC · سوروگیت لنگرشده به
              خروجی‌های واقعی زنجیره (برچسب صادقانه).
            </p>
          </Card>
        ),
      },
          ...
          <Card title="SWAT+ — آماده‌سازی پروژه (pySWATPlus)" icon={<Database size={18} />}>
            ...
            >
              ...
              {swat?.summary?.run_requires_executable === true && (
                <>
                  <span style={{ fontSize: '0.85rem' }}>
                    اجرای کامل نیازمند باینری رایگان SWAT+ است:
                  </span>
                  <a
                    href="https://swat.tamu.edu/software/plus/"
                    target="_blank"
                    rel="noreferrer"
                    style={{ fontSize: '0.8rem', color: 'var(--color-primary)' }}
                  >
            ...
            <p style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)', margin: 0 }}>
              پروژه با ورودی‌های واقعی آماده شده: اقلیم ERA5 (بارش/دما/ET0 روزانه)، خاک SoilGrids،
              کاربرد اراضی و توپوگرافی حوزه ({state.catchmentKm2} km²).
            </p>
          </Card>
        ),
      },
      {
        ...
        content: (
          <Card
            title="حسابداری کربن — MRV (Verra VM0032 + KoboToolbox)"
            icon={<Globe2 size={18} />}
          >
            <div
              style={{
            ...
            >
              ...
              >
                ...
                <input
                  ...
                  min="1"
                  value={mrvArea}
                  onChange={(e) => setMrvArea(Number(e.target.value))}
                  style={{
              ...
              >
                ...
                <select
                  value={mrvPractice}
                  onChange={(e) => setMrvPractice(e.target.value)}
                  style={{
                    padding: '0.45rem 0.7rem',
                    borderRadius: 9,
                    border: '1px solid var(--color-border)',
            ...
            {mrv && typeof mrv.error === 'string' && mrv.error ? (
              <p style={{ color: '#ef4444', fontSize: '0.85rem' }}>⚠️ {mrv.error}</p>
            ) : null}
                  ...
                  <div>
                    ...
                    >
                      ...
                      <span style={{ fontSize: '0.78rem', color: 'var(--color-text-secondary)' }}>
                        {negative
                          ? 'افت کربن (سناریوی پایه در حال انتشار است — مداخله لازم)'
                          : 'ترسیب کربن خالص'}
                      </span>
                    </div>
                    <div
                    ...
                    >
                      <Field
                        label="SOC اولیه"
                        value={`${typeof c.soc_initial_t_ha === 'number' ? c.soc_initial_t_ha.toFixed(1) : '—'} t C/ha`}
                      />
                      <Field
                        label="SOC نهایی"
                        value={`${typeof c.soc_final_t_ha === 'number' ? c.soc_final_t_ha.toFixed(1) : '—'} t C/ha`}
                      />
                      <Field
                        label="تغییر سالانه"
                        value={`${typeof c.delta_soc_t_ha_yr === 'number' ? c.delta_soc_t_ha_yr.toFixed(3) : '—'} t C/ha`}
                      />
                      <Field
                        label="به ازای هکتار"
                        value={`${typeof c.delta_co2e_ha === 'number' ? c.delta_co2e_ha.toFixed(2) : '—'} tCO2e/ha`}
                      />
                      <Field
                        label="مساحت"
                        value={`${typeof c.area_ha === 'number' ? c.area_ha : '—'} ha`}
                      />
                    ...
                    >
                      متد: {String(c.methodology ?? '—')} · تبدیل IPCC (×3.667) · زنجیره RothC:{' '}
                      {String(mrv.rothc_chain_id ?? '—').slice(0, 8)}
                    </p>
                    <p
                      style={{
                        fontSize: '0.78rem',
                    ...
                    >
                      KoboToolbox: {String(kobo.status ?? 'skipped')}
                      {kobo.status === 'requires_credentials'
                        ? ' — با افزودن KOBO_TOKEN/KOBO_FORM_ID به .env، نمونه‌های میدانی SOC خوانده می‌شوند'
                        : ''}
                    </p>
                  </div>
                );
            ...
            {!mrv && !mrvLoading && (
              <p style={{ fontSize: '0.82rem', color: 'var(--color-text-secondary)', margin: 0 }}>
                بودجه کربن خاک بر اساس زنجیره واقعی (RothC-26.3 با ERA5/SoilGrids) + داده میدانی
                اختیاری KoboToolbox؛ خروجی برآورد مدل است و گواهی Verra/Gold Standard نیازمند
                مستندات کامل متدولوژی است.
              </p>
            )}
          </Card>
        ),
    ...
    <div className="sci-root">
      ...
      <div className="sci-inputbar">
        ...
        >
          {loading ? (
            <>
              <RefreshCw size={15} className="hub-spin" /> در حال اجرا…
            </>
          ) : (
            <>
              <Play size={15} /> اجرای زنجیره
            </>
          )}
        </button>
        {r?.cache_hit && (
          <span style={{ fontSize: '0.75rem', color: 'var(--color-success, #10b981)' }}>
            ⚡ نتیجه از کش (شناسه: {r.chain_id.slice(0, 8)})
          </span>
        )}
      </div>

      {error && (
        <p style={{ color: '#ef4444', fontSize: '0.9rem', marginBottom: '1rem' }}>⚠️ {error}</p>
      )}
        ...
        {tabs.map((t) => (
          <button
            ...
            style={{
              ...
              fontWeight: 600,
              border:
                tab === t.id ? '1px solid var(--color-primary)' : '1px solid var(--color-border)',
              background:
                tab === t.id
