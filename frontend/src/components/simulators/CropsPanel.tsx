...
export const CropsPanel = memo(function CropsPanel({
  selectedCrop, selectedLivestock, selectedPoultry,
  onSelectCrop, onSelectLivestock, onSelectPoultry
}: Props) {
  ...
  return (
    <div style={{ width: '100%' }}>
      {/* Crops */}
      <div style={cardStyle}>
        <div style={headerStyle}><Sprout size={12} color="#10b981" />{isFa ? 'محصولات' : 'Crops'}</div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
          {data.crops.map(c => (
            <button key={c.id} style={chipStyle(selectedCrop === c.id, '#10b981')}
        ...
        {selectedCrop && (() => {
          ...
          return (
            <div style={{ marginTop: '10px', padding: '10px', background: 'rgba(16,185,129,0.1)', borderRadius: '8px', fontSize: '11px' }}>
              <div style={{ fontWeight: 700, marginBottom: '4px' }}>📊 {isFa ? 'مشخصات' : 'Details'}:</div>
              <div>💧 {isFa ? 'نیاز آبی' : 'Water'}: {c.waterNeed} mm</div>
              <div>🌱 {isFa ? 'عمق ریشه' : 'Roots'}: {c.rootDepth} cm</div>
      ...
      {/* Livestock */}
      <div style={cardStyle}>
        <div style={headerStyle}><Utensils size={12} color="#f59e0b" />{isFa ? 'دام' : 'Livestock'}</div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
          {data.livestock.map(l => (
            <button key={l.id} style={chipStyle(selectedLivestock === l.id, '#f59e0b')}
        ...
        {selectedLivestock && (() => {
          ...
          return (
            <div style={{ marginTop: '10px', padding: '10px', background: 'rgba(245,158,11,0.1)', borderRadius: '8px', fontSize: '11px' }}>
              <div>💧 {isFa ? 'آب روزانه' : 'Water/day'}: {l.waterPerDay} L</div>
              <div>🌾 {isFa ? 'چرا' : 'Grazing'}: {l.grazingArea} ha/head</div>
            </div>
          );
        })()}
      </div>
      ...
      {/* Poultry */}
      <div style={cardStyle}>
        <div style={headerStyle}><Bird size={12} color="#ec4899" />{isFa ? 'طیور' : 'Poultry'}</div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
          {data.poultry.map(p => (
            <button key={p.id} style={chipStyle(selectedPoultry === p.id, '#ec4899')}
