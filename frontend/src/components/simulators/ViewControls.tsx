...
export const ViewControls = memo(function ViewControls({
  viewMode, setViewMode, onZoomIn, onZoomOut, onResetCamera
}: Props) {
  const { i18n } = useTranslation();
  const isFa = i18n.language === 'fa';

  const modes: Array<{ mode: ViewMode; icon: any; label: string; fa: string; hint: string; hintFa: string }> = [
    { mode: '3d', icon: Compass, label: '3D Orbit', fa: 'سه‌بعدی چرخشی', hint: 'Drag to rotate', hintFa: 'درگ برای چرخش' },
    { mode: '2d-top', icon: MapIcon, label: 'Top View', fa: 'از بالا', hint: 'Fixed top', hintFa: 'بالای ثابت' },
    { mode: '2d-side', icon: Camera, label: 'Side View', fa: 'از کنار', hint: 'Profile view', hintFa: 'نمای پروفیل' },
    { mode: 'cross-section', icon: RotateCw, label: 'Cross-Section', fa: 'برش عرضی', hint: 'Cut view', hintFa: 'نمای برش' },
  ];
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          padding: '0 4px 6px 4px', textTransform: 'uppercase',
          letterSpacing: '0.5px', borderBottom: '1px solid rgba(255,255,255,0.1)',
          marginBottom: '4px',
        }}>
          {isFa ? 'حالت نمایش' : 'View Mode'}
        </div>
        {modes.map(({ mode, icon: Icon, label, fa, hint, hintFa }) => (
          <button key={mode} style={btnStyle(viewMode === mode)}
