...
export const OperationsPanel = memo(function OperationsPanel({ state, onChange, terrain }: Props) {
  ...
  const currentPlowing = data.plowingTypes.find(p => p.id === state.plowingType) || data.plowingTypes[0];
  ...
  return (
    <div style={{ width: '100%' }}>
      {/* Plowing Types */}
      <div style={cardStyle}>
        <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.6)', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.5px', display: 'flex', alignItems: 'center', gap: '6px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          {data.plowingTypes.map(p => {
            ...
            return (
              <button key={p.id} onClick={() => update({ plowingType: p.id, customDepth: p.depth })}
                style={{
                  padding: '10px', borderRadius: '10px', textAlign: 'start',
                ...
                <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.5)', display: 'flex', gap: '8px' }}>
        ...
        <div style={{ marginTop: '12px' }}>
          <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.6)', marginBottom: '6px' }}>
            {isFa ? 'عمق سفارشی' : 'Custom Depth'}: <span style={{ color: '#f59e0b', fontWeight: 700 }}>{effectiveDepth}cm</span>
          </div>
      ...
      <div style={cardStyle}>
          ...
          <div>
            ...
            <div style={{
              ...
              <div style={{
                ...
                background: state.avgErosion > 10 ? '#ef4444' : state.avgErosion > 5 ? '#f59e0b' : '#10b981',
        ...
        {state.avgErosion > 10 && (
          <div style={{
            ...
            borderRadius: '8px', fontSize: '11px', color: '#fca5a5',
          }}>
            ⚠️ {isFa ? 'ریسک فرسایش بالا! عملیات حفاظتی پیشنهاد می‌شود' : 'High erosion risk! Conservation practices recommended'}
          </div>
        )}
      </div>
    </div>
