...
import {
  Sun, Cloud, CloudRain, CloudSnow, Droplets,
  Wind, Thermometer, Clock, Calendar
} from 'lucide-react';
...
export const ClimatePanel = memo(function ClimatePanel({ state, onChange, latitude = 35.5 }: Props) {
      ...
      <div style={cardStyle}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '4px' }}>
          {scales.map(s => {
            ...
            return (
              <button key={s.value} onClick={() => update({ timeScale: s.value, timeIndex: 0 })}
                style={{
      ...
      <div style={cardStyle}>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', marginBottom: '6px' }}>
          ...
          <span style={{ color: '#3b82f6', fontWeight: 700 }}>
            ...
            {state.timeScale === 'month' && (isFa
              ? ['فروردین','اردیبهشت','خرداد','تیر','مرداد','شهریور','مهر','آبان','آذر','دی','بهمن','اسفند'][state.timeIndex]
              : ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'][state.timeIndex])}
          </span>
        </div>
        <input type="range" min={0} max={scales.find(s => s.value === state.timeScale)?.max || 11}
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
          <div style={{
            ...
            background: isDaytime ? 'radial-gradient(circle, #fbbf24, #f59e0b)' : 'radial-gradient(circle, #64748b, #1e293b)',
      ...
      <div style={cardStyle}>
        <div style={rowStyle}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={{ color: 'white', fontWeight: 700 }}>{state.temperature.toFixed(1)}°C</span>
        ...
        <div style={rowStyle}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={{ color: 'white', fontWeight: 700 }}>{(state.humidity * 100).toFixed(0)}%</span>
        ...
        <div style={rowStyle}>
          <span style={{ display: 'flex', alignItems: 'center', gap: '6px', color: '#3b82f6' }}>
            {precipType === 'snow' ? <CloudSnow size={14} /> : precipType === 'rain' ? <CloudRain size={14} /> : <Cloud size={14} />}
            {isFa ? 'بارش' : 'Precip'}
          </span>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
      ...
      <div style={cardStyle}>
        ...
        <div style={{ position: 'relative', width: '80px', height: '80px', margin: '0 auto' }}>
          <svg width="80" height="80" viewBox="0 0 80 80">
            <circle cx="40" cy="40" r="35" fill="rgba(255,255,255,0.05)" stroke="rgba(255,255,255,0.2)" strokeWidth="1" />
            <text x="40" y="8" textAnchor="middle" fill="white" fontSize="10">N</text>
            <text x="75" y="43" textAnchor="middle" fill="white" fontSize="10">E</text>
            <text x="40" y="78" textAnchor="middle" fill="white" fontSize="10">S</text>
            <text x="5" y="43" textAnchor="middle" fill="white" fontSize="10">W</text>
