...
export default function RealSiteLoader({
  ...
  return (
    <div style={{
      background: card, backdropFilter: 'blur(10px)', borderRadius: '12px',
      padding: '12px', border: '1px solid rgba(255,255,255,0.1)',
    }}>
      <div style={{
        fontSize: '12px', color: 'rgba(255,255,255,0.6)', marginBottom: '8px',
        fontWeight: 700, textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: '6px',
      }}>
      ...
      >
        {sites.map(s => (
          <option key={s.site_id} value={s.site_id} style={{ background: '#16181f' }}>
            {s.site_id} — {s.admin1_city || s.province || s.country} ({(s.elevation_m ?? 0).toFixed(0)}m)
          </option>
        ))}
      </select>
      ...
      <button
        ...
        style={{
          ...
          fontWeight: 700, fontSize: '12px', cursor: loading ? 'default' : 'pointer',
      ...
      {lastMeta && (
        <div style={{ marginTop: '8px', fontSize: '11px', color: '#81C784', display: 'flex', gap: '6px', alignItems: 'center' }}>
          <CheckCircle size={12} />
      ...
      {error && (
        <div style={{ marginTop: '8px', fontSize: '11px', color: '#fca5a5', display: 'flex', gap: '6px', alignItems: 'center' }}>
