...
export const WatershedFlowMap: React.FC<WatershedFlowMapProps> = ({
  ...
  const sampleFlowData = useMemo(() => {
    ...
    return [
      ...
      {
        source: [lng + 0.05, lat + 0.05],
        target: [lng + 0.02, lat + 0.02],
        value: 3,
        type: 'runoff' as const,
      },
      {
        source: [lng - 0.04, lat - 0.03],
        target: [lng - 0.01, lat - 0.01],
        value: 2,
        type: 'runoff' as const,
      },
      {
        source: [lng + 0.1, lat],
        target: [lng + 0.05, lat - 0.05],
        value: 5,
        type: 'groundwater' as const,
      },
    ];
  }, [flowData, center]);
      ...
      >
        ...
        >
          ...
          <strong style={{ marginLeft: 'auto', color: '#ef4444' }}>
            {sampleFlowData.filter((d) => d.type === 'runoff').reduce((s, d) => s + d.value, 0)}{' '}
            m³/s
          </strong>
        </div>
