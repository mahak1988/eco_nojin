...
export default function MarketplacePanel() {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      let data: { products?: Product[]; detail?: string } = {};
      try {
        data = JSON.parse(text);
      } catch {
        throw new Error(`پاسخ نامعتبر از سرویس بازارچه (${res.status})`);
      }
      if (!res.ok) throw new Error(data.detail || `دریافت محصولات ناموفق بود (${res.status})`);
      setProducts(data.products || []);
    } catch (err) {
      setProducts([]);
      setError(err instanceof Error ? err.message : 'اتصال به بازارچه برقرار نشد');
    } finally {
      setLoading(false);
    }
  };
