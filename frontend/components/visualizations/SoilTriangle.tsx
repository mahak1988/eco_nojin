  ...
  return (
    <div style={{ textAlign: 'center' }}>
      <svg viewBox={`0 0 ${W_SIZE} ${H + 20}`} style={{ maxWidth: '100%', height: 'auto' }}>
        ...
        {/* Labels */}
        <text x={A.x} y={A.y - 5} textAnchor="middle" fill={colors.text} fontSize="12" fontWeight="bold">Clay {c.toFixed(0)}%</text>
        <text x={B.x} y={B.y + 15} textAnchor="start" fill={colors.text} fontSize="12" fontWeight="bold">Sand {sa.toFixed(0)}%</text>
        <text x={C.x} y={C.y + 15} textAnchor="end" fill={colors.text} fontSize="12" fontWeight="bold">Silt {si.toFixed(0)}%</text>
      </svg>
    </div>
  );
}
