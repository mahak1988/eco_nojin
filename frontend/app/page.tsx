...
import {
  ...
  Globe2, Radio, ArrowRight, PlayCircle, Heart,
...
const stats = [
  { value: '40%', key: 'stat_degraded_land', icon: TrendingDown, color: '#f97316' },
  { value: '2B+', key: 'stat_water_stress', icon: Droplet, color: '#0ea5e9' },
  { value: '41%', key: 'stat_drylands', icon: Globe2, color: '#fbbf24' },
  { value: '216M', key: 'stat_climate_migrants', icon: Users, color: '#fb7185' },
];
    ...
    <div dir={direction} style={{ background: colors.bg, minHeight: '100vh' }}>
      ...
      <section style={{
        ...
        <div style={{ maxWidth: '1100px', margin: '0 auto', textAlign: 'center', position: 'relative', zIndex: 1 }}>
          ...
          >
            <span className="gradient-text">{t('home_hero_sub2')}</span>
          </motion.p>
            ...
            {features.map((f) => {
              ...
              return (
                <Link key={f.key} href={`/modules/${f.key}`}>
                  <motion.div
                    variants={item}
                    whileHover={{ y: -8, boxShadow: `0 20px 40px ${f.color}25` }}
                    style={{
