...
import { Bot, Send, User, Mic, MicOff, Volume2, Trash2, Sparkles } from 'lucide-react';
...
export default function AIChatPage() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      role: 'ai',
      content: language === 'fa' 
        ? 'سلام! من دستیار هوشمند اکو نوژین هستم. درباره خاک، آب، اقلیم، فرسایش یا اعتبار کربن بپرسید.'
  ...
  const [streamMode] = useState<'sse' | 'ws'>('sse');
  const messagesEndRef = useRef<HTMLDivElement>(null);
  const recognitionRef = useRef<any>(null);
  
  const currentLang = t('ai_en');
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && 'webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
      ...
      recognitionRef.current.lang = t('ai_en_us');
                    ...
                    {msg.content.split('\n').map((line, i) => (
                      <div key={i} style={{ marginBottom: '4px' }}>
                        {line.startsWith('**') && line.endsWith('**') ? (
                          ...
                          <div style={{ paddingLeft: '12px' }}>• {line.slice(2)}</div>
                        ) : (
                          line || <br />
                        )}
                      </div>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
              ...
              <input
                ...
                aria-label={t('ai_ask_me_anything_about_farming')} placeholder={t('ai_ask_me_anything_about_farming')}
