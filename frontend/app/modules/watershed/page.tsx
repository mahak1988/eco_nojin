...
export default function WatershedModulePage() {
  ...
  const design = async () => {
    setLoading(true);
    setError(null);
    // API schema: {structure_type, slope_pct, area_m2, rainfall_mm}
    const payload = {
      structure_type: params.structure_type,
      slope_pct: params.slope_percent,
      area_m2: params.catchment_area_ha * 10000,
      rainfall_mm: params.rainfall_intensity_mmh,
    };
    const res = await api.post<any>('/api/v1/watershed/design', payload);
    if (res.success) setResult(res.data);
    else {
      setError(res.error || 'Design failed');
      if (typeof res.error !== 'string') console.error('Watershed design error:', res.error);
    }
    setLoading(false);
  };
          ...
          <div style={{ background: colors.cardBg, padding: '24px', borderRadius: '16px', border: `1px solid ${colors.border}` }}>
            ...
            {[
              ...
              <div key={k} style={{ marginBottom: '12px' }}>
                ...
                <input type="number" aria-label={label} step={step} value={(params as any)[k]}
