...
@patch(
    "engine.hydroma.analyses.topography_analysis.rioxarray.open_rasterio"
)  # Mock the file loader
@patch(
    "engine.hydroma.analyses.topography_analysis.TopographyAnalyzer._save_geotiff"
)  # Mock the file saver
def test_topography_analyzer_execute(mock_save_geotiff, mock_open_rasterio, mock_db_session):
    ...
    input_data = TopographyInput(
        site_id="test_site", dem_path="dummy/path.tif", analysis_types=["slope", "aspect"]
    )
