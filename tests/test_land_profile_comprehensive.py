...
def test_land_profile_table_schema(test_db_session):
    ...
    assert (
        columns["id"]["type"].__class__.__name__ == "VARCHAR"
    )  # SQLAlchemy برای String ممکن است VARCHAR استفاده کند
...
def test_land_profile_creation_valid_data(test_db_session):
    ...
    user = User(
        ...
        hashed_password=os.environ.get("HASHED_PASSWORD", "hashed_pw"),
        full_name="John Doe",
    )
    ...
    land_profile = LandProfile(
        name="My Farm", location_lat=35.7, location_lon=51.4, area_ha=5.2, user_id=user.id
    )
...
def test_land_profile_creation_with_defaults(test_db_session):
    ...
    user = User(
        email="farmer2@example.com", hashed_password=os.environ.get("HASHED_PASSWORD", "hashed_pw2")
    )
    test_db_session.add(user)
    test_db_session.commit()
...
def test_land_profile_nullable_fields(test_db_session):
    ...
    user = User(
        email="flexible@example.com", hashed_password=os.environ.get("HASHED_PASSWORD", "pw")
    )
    test_db_session.add(user)
    test_db_session.commit()
...
def test_land_profile_cascading_orphaned_behavior(test_db_session):
    ...
    """
    user = User(
        email="to_be_deleted@example.com", hashed_password=os.environ.get("HASHED_PASSWORD", "pw")
    )
    test_db_session.add(user)
    test_db_session.commit()
    ...
    orphaned_lp = (
        test_db_session.query(LandProfile).filter(LandProfile.id == land_profile.id).first()
    )
    ...
    logger.info(
        f"Orphaned LP Name: {orphaned_lp.name}, User ID: {orphaned_lp.user_id}, Related User: {orphaned_lp.user}"
    )
