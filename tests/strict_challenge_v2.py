...
class StrictTestResult:
    def __init__(
        self, category, test_name, passed, message, severity="info", expected_behavior=None
    ):
...
class StrictChallengeV2:
    ...
    def add(self, category, test_name, passed, message, severity="info", expected=None):
        self.results.append(
            StrictTestResult(category, test_name, passed, message, severity, expected)
        )
        ...
        if self.sdm:
            try:
                ...
                result = self.sdm.h13_soil_fertility_index(
                    soc_pct=1.5,
                    n_pct=0.15,
                    p_available_ppm=30.0,
                    k_available_ppm=200.0,
                    ph=15.0,
                    biology_index=0.5,
                )
        ...
        if self.soe:
            try:
                result = self.soe.h18_growth_duration_optimizer(
                    rain_window_days=100,
                    temp_window_days=120,
                    stress_onset_day=90,
            ...
            try:
                for rain in [5, 10, 50, 100, 200]:
                    ...
                    if eff > rain:
                        ...
                        return
                self.add(cat, "بارش مؤثر ≤ بارش کل", True, "تمام مقادیر بررسی شدند", "info")
            except Exception as e:
            ...
            try:
                ...
                for t in temps:
                    ...
                    if not (0 <= ks <= 1):
                        ...
                        return
                self.add(
                    cat, "Ks ∈ [۰,۱] برای همه دماها", True, f"{len(temps)} دما بررسی شد", "info"
                )
            except Exception as e:
            ...
            try:
                for t_night in [20, 30, 40, 50, 60]:
                    ...
                    if penalty > max_penalty + 0.001:
                        ...
                        return
                self.add(cat, "جریمه شب ≤ سقف", True, "تمام دماها بررسی شدند", "info")
            except Exception as e:
        ...
        if self.sdm:
            try:
                ...
                result = self.sdm.h13_soil_fertility_index(
                    soc_pct=10.0,
                    n_pct=1.0,
                    p_available_ppm=200.0,
                    k_available_ppm=1000.0,
                    ph=7.0,
                    biology_index=1.0,
                )
                fertility = result["fertility_index"]
    ...
    def test_monotonicity(self):
        ...
        if self.dse:
            try:
                ...
                is_monotone = all(
                    ks_values[i] >= ks_values[i + 1] for i in range(len(ks_values) - 1)
                )
                self.add(
                    cat,
                    "Ks یکنوا نزولی با دما",
                    is_monotone,
        ...
        if self.dse:
            try:
                ...
                is_monotone = all(
                    eff_values[i] <= eff_values[i + 1] for i in range(len(eff_values) - 1)
                )
                self.add(
                    cat,
                    "بارش مؤثر یکنوا صعودی با بارش",
                    is_monotone,
        ...
        if self.sdm:
            try:
                ...
                is_monotone = all(
                    awc_values[i] <= awc_values[i + 1] for i in range(len(awc_values) - 1)
                )
                self.add(
                    cat,
                    "AWC یکنوا صعودی با SOC",
                    is_monotone,
        ...
        if self.sdm:
            try:
                ...
                depths = [
                    self.sdm.h10_root_depth_decay(100.0, e, 10)["effective_root_depth_cm"]
                    for e in erosions
                ]
                is_monotone = all(depths[i] >= depths[i + 1] for i in range(len(depths) - 1))
                self.add(
                    cat,
                    "عمق ریشه یکنوا نزولی با فرسایش",
                    is_monotone,
        ...
        if self.soe:
            try:
                ...
                vulns = [
                    self.soe.h19_genetic_vulnerability(d, 50.0, 3)["vulnerability_index"]
                    for d in diversities
                ]
                is_monotone = all(vulns[i] >= vulns[i + 1] for i in range(len(vulns) - 1))
                self.add(
                    cat,
                    "آسیب‌پذیری یکنوا نزولی با تنوع",
                    is_monotone,
        ...
        if self.uke:
            try:
                ...
                scores = [
                    self.uke.h25_local_knowledge_integration(e, 0.7, 0.7, 0.7)["knowledge_score"]
                    for e in experiences
                ]
                is_monotone = all(scores[i] <= scores[i + 1] for i in range(len(scores) - 1))
                self.add(
                    cat,
                    "دانش بومی یکنوا صعودی با تجربه",
                    is_monotone,
        ...
        if self.sdm and self.soe:
            try:
                ...
                soil_result = self.sdm.h13_soil_fertility_index(
                    soc_pct=3.0,
                    n_pct=0.3,
                    p_available_ppm=50.0,
                    k_available_ppm=300.0,
                    ph=7.0,
                    biology_index=0.8,
                )
