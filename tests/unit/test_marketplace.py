...
from engine.hydroma.marketplace.order_management import OrderManager, get_order_manager
from engine.hydroma.marketplace.product_catalog import ProductCatalog, get_catalog
from engine.hydroma.marketplace.traceability import TraceabilitySystem
...
class TestModels:
    ...
    def test_product_can_fulfill_order(self):
        ...
        assert product.can_fulfill_order(50)
        assert not product.can_fulfill_order(5)  # Below minimum
        assert not product.can_fulfill_order(150)  # Above available
