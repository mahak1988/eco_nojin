...
class TestWhatIfEngine:
    ...
    def test_adapted_crop_performs_better_under_drought(self):
        ...
        wheat_yield = next(s for s in result["scenarios"] 
                          if s["scenario_name"] == "Wheat_Drought")
        millet_yield = next(s for s in result["scenarios"] 
                           if s["scenario_name"] == "Millet_Drought")
