...
from engine.hydroma.cpp_bridge.indices_fast import (
    ndvi_fast, evi_fast, savi_fast, nbr_fast, is_numba_available
)
from engine.hydroma.cpp_bridge.soil_physics_fast import (
    soil_water_content, hydraulic_conductivity, SOIL_PARAMETERS
)
from engine.hydroma.cpp_bridge.hydrology_fast import (
    route_flood_wave, route_multi_reach, compute_wave_parameters
)
...
def test_flood_routing_produces_outflow():
    ...
    assert result["peak_outflow"] > 0.1,         f"Expected non-zero outflow, got {result['peak_outflow']:.4f}"
    
    # Attenuation should occur (peak_out < peak_in)
    assert 0 < result["attenuation_ratio"] < 1.0,         f"Attenuation ratio should be in (0,1), got {result['attenuation_ratio']:.3f}"
...
def test_flood_routing_mass_conservation():
    ...
    assert 0.8 < result["mass_balance"] < 1.2,         f"Mass balance should be ~1, got {result['mass_balance']:.3f}"
