...
REGIONS = {
    ...
    "R16_gulf_coastal": {
        ...
        "soil_type": "Solonchak",
        "soil_texture": "sandy_clay",
        "soc_pct": 0.5,
        "ph": 8.3,
        "ec_ds_m": 15.0,
        "awc_mm_m": 60,
        "ksat_mm_h": 8.0,
        "groundwater_depth_m": 2,
        "aquifer_thickness_m": 15,
        "irrigation_available": True,
        "irrigation_efficiency": 0.30,
...
class Scientist25Challenge:
    ...
    def test_hydrologist(self, region_id, region):
        ...
        if texture == "clay" and ksat > 5:
            self.add_result(
                region_id,
                scientist,
                "Ksat با بافت رسی سازگار نیست",
                False,
                f"Ksat={ksat}mm/h برای {texture} بسیار بالاست",
                "critical",
            )
            ...
            self.add_result(
                region_id,
                scientist,
                "Ksat با بافت شنی سازگار نیست",
                False,
                f"Ksat={ksat}mm/h برای {texture} بسیار پایین است",
                "critical",
            )
        else:
            self.add_result(
                region_id,
                scientist,
                "سازگاری Ksat با بافت",
                True,
                f"Ksat={ksat}mm/h، بافت={texture}",
                "info",
            )
        ...
        if gw_depth < 0:
            self.add_result(
                region_id, scientist, "عمق آب زیرزمینی منفی", False, f"عمق={gw_depth}m", "critical"
            )
        elif gw_depth < 1:
            self.add_result(
                region_id,
                scientist,
                "آب زیرزمینی بسیار سطحی",
                True,
                f"عمق={gw_depth}m - احتمال غرقابی",
                "warning",
            )
        else:
            self.add_result(
                region_id, scientist, "عمق آب زیرزمینی منطقی", True, f"عمق={gw_depth}m", "info"
            )
        ...
        if texture == "sand" and awc > 80:
            self.add_result(
                region_id,
                scientist,
                "AWC برای خاک شنی غیرواقعی",
                False,
                f"AWC={awc}mm/m برای {texture}",
                "critical",
            )
        else:
            self.add_result(region_id, scientist, "AWC در محدوده", True, f"AWC={awc}mm/m", "info")
    ...
    def test_climatologist(self, region_id, region):
        ...
        if koppen.startswith("A") and tmin < 15:
            self.add_result(
                region_id,
                scientist,
                "اقلیم A با Tmin<15 ناسازگار",
                False,
                f"کوپن={koppen}, Tmin={tmin}",
                "critical",
            )
            ...
            self.add_result(
                region_id,
                scientist,
                "اقلیم E با Tmax>15 ناسازگار",
                False,
                f"کوپن={koppen}, Tmax={tmax}",
                "critical",
            )
        else:
            self.add_result(
                region_id, scientist, "سازگاری کوپن با دما", True, f"کوپن={koppen}", "info"
            )
        ...
        if koppen == "ET" and season > 90:
            self.add_result(
                region_id,
                scientist,
                "فصل رشد توندرا غیرواقعی",
                False,
                f"فصل رشد={season} روز",
                "critical",
            )
        else:
            self.add_result(
                region_id, scientist, "فصل رشد منطقی", True, f"فصل رشد={season} روز", "info"
            )

        rain_cv = region.get("rain_cv", 0.3)
        if rain_cv > 0.55:
            self.add_result(
                region_id, scientist, "بارش بسیار نامنظم", True, f"CV={rain_cv}", "warning"
            )
        else:
            self.add_result(
                region_id, scientist, "CV بارش در محدوده", True, f"CV={rain_cv}", "info"
            )
    ...
    def test_soil_scientist(self, region_id, region):
        ...
        if soc < 0 or soc > 15:
            self.add_result(
                region_id, scientist, "SOC خارج از محدوده", False, f"SOC={soc}%", "critical"
            )
        elif soc < 0.5:
            self.add_result(
                region_id,
                ...
                "SOC بسیار پایین",
                True,
                f"SOC={soc}% - خاک تخریب‌شده",
                "warning",
            )
        else:
            self.add_result(region_id, scientist, "SOC در محدوده", True, f"SOC={soc}%", "info")

        if ph < 3 or ph > 11:
            self.add_result(
                region_id, scientist, "pH خارج از محدوده", False, f"pH={ph}", "critical"
            )
        else:
            self.add_result(region_id, scientist, "pH در محدوده", True, f"pH={ph}", "info")
        ...
        if self.dse:
            try:
                ...
                self.add_result(
                    ...
                    "Ks حرارتی",
                    0 <= h04_ks <= 1,
                    f"Ks={h04_ks:.3f} در Tmax={tmax}C",
                    "critical" if not (0 <= h04_ks <= 1) else "info",
                )
            except Exception as e:
                self.add_result(
                    region_id, scientist, "Ks حرارتی", False, f"خطا: {str(e)[:50]}", "critical"
                )
    ...
    def test_salinity_expert(self, region_id, region):
        ...
        if ec > 8 and texture == "clay":
            self.add_result(
                ...
                "شوری بالا + خاک رسی",
                True,
                "نیاز به زهکشی قبل از آبشویی",
                "warning",
            )

        if ph > 8.5 and ec > 4:
            self.add_result(
                region_id, scientist, "شوری-سدیک احتمالی", True, f"pH={ph}, EC={ec}", "warning"
            )
        ...
        if self.sdm:
            try:
                ...
                self.add_result(
                    region_id, scientist, "پیش‌بینی شوری", False, f"خطا: {str(e)[:50]}", "critical"
                )
    ...
    def test_geneticist(self, region_id, region):
        ...
        if len(crops) <= 1:
            self.add_result(
                region_id, scientist, "تنوع ژنتیکی پایین", True, f"تعداد={len(crops)}", "warning"
            )
    ...
    def test_economist(self, region_id, region):
        ...
        if irrigation and (efficiency < 0.1 or efficiency > 0.95):
            self.add_result(
                region_id,
                scientist,
                "راندمان آبیاری غیرواقعی",
                False,
                f"راندمان={efficiency}",
                "critical",
            )
            ...
            self.add_result(
                region_id, scientist, "راندمان آبیاری منطقی", True, f"راندمان={efficiency}", "info"
            )
        ...
        if self.uke:
            try:
                ...
                self.add_result(
                    region_id,
                    scientist,
                    "عدم قطعیت مونت‌کارلو",
                    cv >= 0,
                    f"CV={cv:.3f}, P10={result['p10_t_ha']}, P90={result['p90_t_ha']}",
    ...
    def test_erosion_expert(self, region_id, region):
        ...
        if "slope" in challenges or "extreme_slope" in challenges:
            self.add_result(region_id, scientist, "ریسک فرسایش بالا", True, "شیب > 30%", "warning")

        if soc < 0.5:
            self.add_result(
                region_id, scientist, "SOC پایین + فرسایش‌پذیری", True, f"SOC={soc}%", "warning"
            )
        ...
        if self.sdm:
            try:
                ...
                self.add_result(
                    region_id, scientist, "کاهش عمق ریشه", False, f"خطا: {str(e)[:50]}", "critical"
                )
