...
# -*- coding: utf-8 -*-
"""
Fix: Dynamic admin/index.ts based on actual files
=================================================
Scans src/pages/admin/ directory and creates barrel file
with ONLY exports for files that actually exist.
"""
...
def main():
    print("")
    print("=" * 70)
    print("  Fix: Dynamic admin/index.ts (file-aware)")
    print("=" * 70)
    print("")
    ...
    # Subdirectories (crypto, telegram, etc.)
    for subdir in sorted(subdirs):
        # Special naming for subdirectories
        if subdir == "crypto":
            exports.append("export { default as CryptoPaymentWidget } from './crypto/CryptoPaymentWidget';")
        elif subdir == "telegram":
            exports.append("export { default as TelegramManager } from './telegram/TelegramManager';")
        else:
    ...
    if app_file.exists():
        ...
        # Extract available export names
        available = set()
        for exp in exports:
            match = exp.match(r'export\s+\{\s*(?:default\s+as\s+)?(\w+)')
            if match:
    ...
    if build_ok:
        ...
        try:
            ...
            msg = (
                "fix(admin): dynamic barrel file based on actual files\\n\\n"
                ...
                "- Removed ThemeProvider (doesn't exist in admin/)\\n"
