...
def ensure_user():
    with hub.get_session() as s:
        ...
        uid = 'e2e-buyer-0001'
        s.execute(text("INSERT INTO users (id, email, hashed_password, full_name, role, language, is_email_verified, created_at, updated_at) VALUES (:i, :e, :h, :f, 'buyer', 'fa', 0, datetime('now'), datetime('now'))"),
...
def main():
    ...
    with hub.get_session() as s:
        from services.marketplace.models import (
            MarketplaceProduct, MarketplaceProductStatus, MarketplaceSeller,
        )
        if s.get(MarketplaceSeller, 'e2e-seller-0001') is None:
            s.add(MarketplaceSeller(id='e2e-seller-0001', user_id=user_id, village_id='v-e2e',
                                    shop_name='فروشگاه E2E', status='active', is_verified=True))
        if s.get(MarketplaceProduct, 'e2e-product-0001') is None:
            s.add(MarketplaceProduct(id='e2e-product-0001', seller_id='e2e-seller-0001',
                                     name='عسل طبیعی E2E', slug='e2e-honey',
                                     description='محصول واقعی تست انتها-به-انتها',
                                     category='honey', price=350000, stock=100, unit='kg',
                                     village_id='v-e2e', organic=True,
    ...
    r = client.post('/api/v1/marketplace/orders', json={'product_id': pid, 'buyer_name': 'E2E Buyer', 'quantity_kg': 10}, headers=hdr())
    ...
    r = client.post('/api/v1/marketplace/cart', json={'items': [{'product_id': pid, 'quantity': 1}]}, headers=hdr())
    step(r.status_code == 200, 'authenticated cart add -> 200')

    r = client.post('/api/v1/marketplace/payments', json={'order_id': order_id, 'amount': total, 'payment_method': 'bank', 'description': 'E2E'}, headers=hdr())
    ok = r.status_code == 200
    payment = r.json() if ok else {}
    payment_id = payment.get('id', '')
    step(ok and payment.get('status') == 'awaiting_verification', 'bank payment initiated: ' + payment_id)

    r = client.post('/api/v1/marketplace/payments/%s/confirm' % payment_id, json={'ref_id': 'TRK-E2E-001'}, headers=hdr())
    ...
    print('')
    print('E2E RESULT: %d/%d passed%s' % (len(results) - len(failed), len(results), ('' if not failed else '  FAILED: ' + '; '.join(failed))), flush=True)
