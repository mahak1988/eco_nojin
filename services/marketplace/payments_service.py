...
def _http_json(url: str, payload: dict) -> dict:
    ...
    with urllib.request.urlopen(req, timeout=20) as resp:  # noqa: S310 (fixed https hosts)
...
class PaymentGateways:
    ...
    def create(self, order_id: str, user_id: str, gateway: str, amount: float,
               description: str = '', callback_path: str = '/api/v1/marketplace/payments/zarinpal/callback') -> MarketplacePayment:
        if gateway not in VALID_GATEWAYS:
            raise PaymentError(f'unknown gateway: {gateway}')
        if amount <= 0:
            raise PaymentError('amount must be positive')

        payment = MarketplacePayment(order_id=order_id, user_id=user_id, gateway=gateway, amount=amount)
        if gateway == 'zarinpal':
            ...
            if not link:
                raise PaymentError('درگاه بینالمللی پیکربندی نشده است (INTL_PAYMENT_CHECKOUT_URL خالی است)')
    ...
    def _zarinpal_request(self, amount: float, description: str, callback_path: str) -> dict[str, Any]:
    ...
    def verify(self, payment: MarketplacePayment, ref_id: str | None = None,
               card_pan: str | None = None) -> MarketplacePayment:
        ...
        if payment.gateway == 'zarinpal':
            ...
            raw = _http_json(verify_url, {
                'merchant_id': merchant, 'amount': int(payment.amount), 'authority': payment.authority,
            })
            body = raw.get('data') or {}
...
class EscrowService:
    ...
    def hold(self, payment: MarketplacePayment, seller_id: str | None = None,
             marketplace_id: str | None = None, actor_id: str | None = None) -> MarketplaceEscrowEntry:
        if payment.escrow_status != 'none':
            raise PaymentError('escrow already opened for this payment')
        entry = MarketplaceEscrowEntry(
            payment_id=payment.id, order_id=payment.order_id, entry_type='hold',
            amount=payment.amount, currency=payment.currency,
            seller_id=seller_id, marketplace_id=marketplace_id, actor_id=actor_id,
    ...
    def release(self, payment_id: str, actor_id: str | None = None,
                note: str = 'released on delivery confirmation') -> MarketplaceEscrowEntry:
        ...
        entry = MarketplaceEscrowEntry(
            payment_id=payment.id, order_id=payment.order_id, entry_type='release',
            amount=payment.amount, currency=payment.currency, actor_id=actor_id, note=note,
        )
        self.db.add(entry)
    ...
    def refund(self, payment_id: str, actor_id: str | None = None,
               note: str = 'refunded to buyer') -> MarketplaceEscrowEntry:
        payment = self.db.get(MarketplacePayment, payment_id)
        if payment is None:
            raise LookupError(f'payment not found: {payment_id}')
        if payment.escrow_status != 'held':
            raise PaymentError(f'escrow is not held (status={payment.escrow_status})')
        entry = MarketplaceEscrowEntry(
            payment_id=payment.id, order_id=payment.order_id, entry_type='refund',
            amount=payment.amount, currency=payment.currency, actor_id=actor_id, note=note,
        )
        self.db.add(entry)
