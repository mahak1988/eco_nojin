...
@router.post("")
def create_dispute(
    payload: DisputeCreate, db: Session = Depends(get_db), user=Depends(require_user)
) -> dict[str, Any]:
...
def resolve_dispute(
    ...
    try:
        d = DisputeService(db).resolve(
            dispute_id, payload.resolved_by, payload.resolution, payload.outcome
        )
        return {"ok": True, "dispute": d.to_dict()}
...
def escalate_dispute(
    dispute_id: str, actor: str, db: Session = Depends(get_db), user=Depends(require_user)
) -> dict[str, Any]:
    try:
