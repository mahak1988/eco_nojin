...
class DisputeService:
    ...
    def create(
        ...
        self.db.add(
            DisputeEvent(
                dispute_id=dispute.id, event="opened", actor=complainant_id, note=description[:200]
            )
        )
        self.db.commit()
        return dispute
    ...
    def add_event(
        self, dispute_id: str, event: str, actor: str, note: str | None = None
    ) -> DisputeEvent:
    ...
    def resolve(
        self, dispute_id: str, resolved_by: str, resolution: str, outcome: str = "resolved"
    ) -> Dispute:
        ...
        self.db.add(
            DisputeEvent(
                dispute_id=d.id, event="resolved", actor=resolved_by, note=resolution[:200]
            )
        )
        self.db.commit()
        return d
