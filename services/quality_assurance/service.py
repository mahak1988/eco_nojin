...
class QualityService:
    ...
    def create_inspection(self, product_id: str, inspection_type: str,
                          marketplace_id: str | None = None,
                          inspector_id: str | None = None) -> QualityInspection:
        if inspection_type not in ('harvest', 'packaging', 'delivery'):
            raise ValueError(f'unknown inspection type: {inspection_type}')
        insp = QualityInspection(product_id=product_id, inspection_type=inspection_type,
                                 marketplace_id=marketplace_id, inspector_id=inspector_id)
        self.db.add(insp)
        self.db.commit()
        self.db.refresh(insp)
        return insp

    def record_result(self, inspection_id: str, score: float, notes: str | None = None,
                      inspector_id: str | None = None) -> dict[str, Any]:
        ...
        if insp.status == 'passed' and score >= CERTIFICATE_MIN_SCORE:
            cert = QualityCertificate(inspection_id=insp.id, product_id=insp.product_id,
                                      standard='econojin-quality', issued_by=insp.inspector_id,
    ...
    def list(self, product_id: str | None = None, limit: int = 50) -> list[QualityInspection]:
        ...
        if product_id:
            stmt = select(QualityInspection).where(QualityInspection.product_id == product_id).limit(limit)
