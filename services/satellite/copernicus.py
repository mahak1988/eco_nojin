...
class CopernicusClient:
    ...
    def search_stac(
        ...
        """Search Sentinel-2 L2A STAC items covering a point (newest first)."""
        self._ensure_configured()
    ...
    async def sample_bands(
        self, scene: Scene, latitude: float, longitude: float
    ) -> Dict[str, Any]:
    ...
    async def analyze_location(
        ...
        scene = usable[0]
        try:
            bands = await self.sample_bands(scene, latitude, longitude)
