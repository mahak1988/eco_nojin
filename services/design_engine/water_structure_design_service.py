...
class StructureDesignInput(BaseModel):
    ...
    max_flow_m3s: float = Field(
        ..., gt=0, description="Estimated maximum flow rate (cubic meters per second)"
    )
...
class StructureDesignOutput(BaseModel):
    ...
    geometry_geojson: dict = Field(
        ..., description="GeoJSON representation of the structure layout"
    )
    material_estimate: dict[str, float] = Field(
        ..., description="Estimated materials needed (e.g., {'stone': 100, 'sand': 50})"
    )  # kg or m3
...
class StructureDesigner:
    ...
    def __init__(self):
        ...
        self.cost_database = {
            "check_dam": {
                "stone_per_m3": 1600,
                "cost_per_ton_stone": 10,
                "cost_per_m3_construction": 50,
            },
    ...
    def _design_check_dam(self, input_data: StructureDesignInput) -> StructureDesignOutput:
        ...
        cost_construction = (
            volume_fill_m3 * self.cost_database["check_dam"]["cost_per_m3_construction"]
        )
    ...
    def _design_half_moon(self, input_data: StructureDesignInput) -> StructureDesignOutput:
        """Design a half-moon (crescent-shaped) water harvesting structure.

        Half-moon structures are semi-circular pits dug on contour lines
        to capture and retain surface runoff for groundwater recharge.
        """
        logger.info("Designing a half-moon structure...")

        # Half-moon dimensions based on contributing area
        radius_m = min(2.0, (input_data.area_ha * 10) ** 0.5)  # Empirical
        depth_m = 0.8
        width_m = 2 * radius_m
        length_m = radius_m  # Half-circle

        # Volume of half-cylinder
        volume_exc_m3 = 0.5 * 3.14159 * (radius_m**2) * depth_m

        # Cost estimation
        cost_exc = volume_exc_m3 * self.cost_database["half_moon"]["excavation_cost_per_m3"]
        total_cost = cost_exc

        # Geometry: half-circle polygon
        center_point = Point(input_data.site_location_lon, input_data.site_location_lat)
        buffer_dist = 0.00001 * radius_m
        half_moon_poly = center_point.buffer(buffer_dist, resolution=16).boundary

        gdf = gpd.GeoDataFrame([1], geometry=[half_moon_poly], crs="EPSG:4326")
        geojson_geom = json.loads(gdf.to_json())["features"][0]["geometry"]

        return StructureDesignOutput(
            design_id=f"HM-{uuid4().hex[:8]}",
            geometry_geojson=geojson_geom,
            material_estimate={"excavated_material_m3": round(volume_exc_m3, 2)},
            cost_estimate_usd=round(total_cost, 2),
            design_summary={
                ...
                "volume_m3": round(volume_exc_m3, 2),
            },
        )

    def _design_retention_pond(self, input_data: StructureDesignInput) -> StructureDesignOutput:
        """Design a retention pond for stormwater management.

        Retention ponds capture and store storm runoff, releasing it slowly
        to prevent downstream flooding and support groundwater recharge.
        """
        logger.info("Designing a retention pond...")

        # Pond dimensions based on contributing area and flow
        volume_m3 = input_data.max_flow_m3s * 3600 * 2  # 2-hour detention
        surface_area_m2 = volume_m3 / 1.5  # Average depth 1.5m
        length_m = (surface_area_m2**0.5) * 1.5
        width_m = surface_area_m2 / length_m
        depth_m = 1.5

        # Cost estimation
        excavation_vol = volume_m3
        lining_area = surface_area_m2 + 2 * depth_m * (length_m + width_m)  # Bottom + sides
        cost_exc = excavation_vol * self.cost_database["retention_pond"]["excavation_cost_per_m3"]
        cost_liner = lining_area * self.cost_database["retention_pond"]["liner_cost_per_m2"]
        total_cost = cost_exc + cost_liner

        # Geometry: rectangular pond
        center_point = Point(input_data.site_location_lon, input_data.site_location_lat)
        buffer_dist = 0.00001 * length_m
        pond_poly = center_point.buffer(buffer_dist, cap_style="square")

        gdf = gpd.GeoDataFrame([1], geometry=[pond_poly], crs="EPSG:4326")
        geojson_geom = json.loads(gdf.to_json())["features"][0]["geometry"]

        return StructureDesignOutput(
            design_id=f"RP-{uuid4().hex[:8]}",
            geometry_geojson=geojson_geom,
            material_estimate={
                "excavated_material_m3": round(excavation_vol, 2),
                "liner_m2": round(lining_area, 2),
            },
            cost_estimate_usd=round(total_cost, 2),
            design_summary={
                "length_m": round(length_m, 2),
    ...
    def execute(self, input_data: StructureDesignInput) -> StructureDesignOutput:
        ...
        if input_data.structure_type == "check_dam":
            ...
            raise ValueError(
                f"Design for structure type '{input_data.structure_type}' is not yet implemented."
            )
