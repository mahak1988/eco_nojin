...
def _db_context(db: Session) -> dict[str, Any]:
    ...
    try:
        errors_24h = db.query(models.ErrorLog).filter(models.ErrorLog.created_at >= day_ago).count()
...
def _cmd_security(db: Session) -> dict[str, Any]:
    ...
    return {
        "answer": f"امنیت ۲۴ ساعت — ورودها: {sec['ورودها']} | کل رویدادهای حسابرسی: {sec['کل_حسابرسی']}"
    }
...
def _cmd_model(parts: list[str]) -> dict[str, Any]:
    ...
    return {
        "answer": f"مدل فعلی: {_state['alias']} ({_state['model']}) — گزینه‌ها: {', '.join(MODEL_REGISTRY)}"
    }
...
async def _cmd_run(db: Session, parts: list[str]) -> dict[str, Any]:
    ...
    if len(parts) < 3:
        return {
            "answer": "استفاده: /run <motor> <site_id> [پارامترها] — مثال: /run aquacrop SITE103 2022-11-01 2023-06-30 wheat"
        }
...
async def ask_admin_assistant(
    ...
    if q.startswith("/"):
        ...
        if cmd == "status":
            ...
            return {
                "answer": json.dumps(ctx, ensure_ascii=False, default=str, indent=1),
                "provider": "command-router",
                "model": _state["model"],
                "context": ctx,
            }
        ...
        if cmd == "run":
            try:
                ...
                return {
                    "answer": f"خطای اجرای موتور: {str(exc)[:200]}",
                    "provider": "command-router",
                }
