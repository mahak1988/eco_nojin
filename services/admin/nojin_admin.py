...
async def get_dashboard_overview():
    return SystemHealth(
        total_projects=127,
        active_projects=98,
        total_area_ha=15240.5,
        total_carbon_tons=45720.8,
        total_credits_issued=38500,
        revenue_usd=962500.0,
...
async def list_projects(status_filter: str | None = None, limit: int = 50):
    projects = [
        ProjectStatus(
            project_id="PRJ-001",
            name="Khuzestan Restoration",
            area_ha=250.0,
            soil_type="Saline",
            status="active",
            start_date=date(2026, 1, 15),
            carbon_sequestered_t=1250.5,
            credits_issued=1100,
            credits_pending=150,
        ),
        ProjectStatus(
            project_id="PRJ-002",
            name="Yazd Desert Project",
            area_ha=500.0,
            soil_type="Desert Sandy",
            status="active",
            start_date=date(2026, 2, 1),
            carbon_sequestered_t=2850.0,
            credits_issued=2500,
            credits_pending=350,
        ),
    ]
...
async def get_material_inventory():
    return [
        MaterialInventory(
            material_code="MIN-011",
            name="Zeolite",
            stock_tons=500.0,
            monthly_consumption_tons=50.0,
            months_of_stock=10.0,
            reorder_point_tons=100.0,
            status="ok",
        ),
        MaterialInventory(
            material_code="ANM-027",
            name="Sheep Manure",
            stock_tons=80.0,
            monthly_consumption_tons=60.0,
            months_of_stock=1.3,
...
async def get_monthly_report(year: int, month: int):
    return {
        ...
        "projects_active": 98,
        "new_projects": 12,
        "area_treated_ha": 1250.5,
        "carbon_sequestered_t": 3750.8,
        "credits_issued": 3200,
        "revenue_usd": 80000.0,
        "water_saved_m3": 2500000,
    }
