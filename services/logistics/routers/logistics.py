...
def get_shipment(shipment_id: str, db: Session = Depends(get_db)) -> dict[str, Any]:
    try:
        ...
        return {'shipment': {'id': sh.id, 'order_id': sh.order_id, 'status': sh.status,
                             'carrier': sh.carrier, 'tracking_code': sh.tracking_code}}
    except LookupError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc


@router.post('/shipments/{shipment_id}/status')
def update_status(shipment_id: str, payload: StatusUpdate,
                  db: Session = Depends(get_db), user=Depends(require_user)) -> dict[str, Any]:
    try:
        sh = LogisticsService(db).update_status(shipment_id, payload.status, payload.actor, payload.note)
        return {'ok': True, 'id': sh.id, 'status': sh.status}
