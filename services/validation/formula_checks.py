...
def _run(check_id: str, label: str, kind: str, source: str, unit: str,
         fn: Callable[[], tuple[Any, Any, float]], note: str = '') -> CheckResult:
    try:
    ...
    except Exception as exc:  # noqa: BLE001 - surfaced as a failed check, never silent
        return CheckResult(check_id, label, kind, source, 'n/a', f'ERROR: {exc}', 0.0, unit, False, note)
...
def run_all() -> dict[str, Any]:
    ...
    checks.append(_run(
        'runoff_rational_identity', 'Runoff V = C·P·A (C=0.45, P=40mm, A=2ha)', 'identity',
        'rational-method unit identity (SCS-style)', 'm³',
    ...
    checks.append(_run(
        'vg_saturation_identity', 'van Genuchten θ(0) = θs', 'identity',
        'van Genuchten 1980 boundary identity', 'cm³/cm³',
        lambda: (vg['theta_s'], van_genuchten_theta(0.0, vg['theta_r'], vg['theta_s'], vg['alpha'], vg['n']), 1e-6),
    ))

    # 3 — loam θs inside Carsel & Parrish (1988) range
    checks.append(_run(
        'vg_loam_params', 'loam θs within Carsel & Parrish range', 'reference',
        'Carsel & Parrish 1988 (θs≈0.43; α=0.036; n=1.56)', 'cm³/cm³',
    ...
    checks.append(_run(
        'ptf_awc_loam', 'Pedotransfer AWC(loam) inside USDA range', 'reference',
        f"Saxton & Rawls 2006 + {usda_awc.get('source', 'USDA reference data')}", 'mm/m',
    ...
    checks.append(_run(
        'awc_cross_family_band', 'AWC(loam): VG vs USDA/PTF within documented band', 'consistency',
        'van Genuchten (physics) vs Saxton-Rawls PTF; accepted band [0.5, 1.5]', 'ratio',
        awc_ratio,
        note=(f'VG-derived={phys_awc_mm_m:.1f} vs PTF={ptf_awc_mm_m:.1f} mm/m — VG underestimates '
              f'loam FC; documented model-family difference'),
    ))

    # 6 — USDA texture triangle known points
    def texture_points() -> tuple[Any, Any, float]:
        points = [((40.0, 40.0, 20.0), 'loam'), ((65.0, 25.0, 10.0), 'sandy_loam'),
                  ((10.0, 30.0, 60.0), 'clay'), ((5.0, 85.0, 10.0), 'silt')]
        bad = [f'{s}/{si}/{c}:{classify_texture(s, si, c)}≠{exp}' for (s, si, c), exp in points
               if classify_texture(s, si, c) != exp]
        return ('all 4 USDA points', 'all 4 USDA points' if not bad else '; '.join(bad), 0.0)
    checks.append(_run(
        'usda_texture_points', 'USDA triangle: 4 known points classify correctly', 'reference',
    ...
    checks.append(_run(
        'fao56_hargreaves', 'FAO-56 eq.52 Hargreaves ET0', 'identity',
        'Allen et al. 1998 (FAO-56 eq. 52; Ra→mm via 0.408)', 'mm/day',
        hargreaves,
    ))

    # 8 — RothC temperature modifier identity
    checks.append(_run(
        'rothc_temp_factor', 'RothC f(T) at 15 °C', 'identity',
        'Coleman & Jenkinson 1996 (f(T)=e^(0.047T−0.86))', '',
        lambda: (math.exp(0.047 * 15 - 0.86), ECSI.temperature_factor(15.0), 1e-9),
    ))

    # 9 — ECSI mass balance identity
    def ecsi_balance() -> tuple[float, float, float]:
        out = ECSI().compute(initial_soc_t_ha=40.0, carbon_input_t_ha=3.0, t_mean_c=15.0,
                             rainfall_mm=500.0, evaporation_mm=700.0, clay_fraction=0.3,
                             land_use='arable')
        return (3.0 - out['total_decomposition_t_ha'], out['delta_soc_t_ha_yr'], 1e-9)
    checks.append(_run(
        'ecsi_mass_balance', 'ECSI ΔSOC = input − decomposition', 'identity',
        'mass-balance identity (RothC structure)', 't/ha/yr',
        ecsi_balance,
    ))

    # 10 — Beer-Lambert fIPAR identity
    checks.append(_run(
        'hyrue_beer_lambert', 'HY-RUE fIPAR = 1−e^(−k·LAI)', 'identity',
        'Monteith 1977 (Beer-Lambert)', '',
        lambda: (1 - math.exp(-0.65 * 2.5), float(HYRUE().f_ipar(np.array([2.5]), 0.65)[0]), 1e-9),
    ))

    # 11 — discounting + payback conventions
    checks.append(_run(
        'npv_identity', 'NPV of −100 at t=0 equals −100', 'identity',
        'discounting identity', '',
        lambda: (-100.0, calculate_npv([-100.0], 0.1), 1e-9),
    ))
    checks.append(_run(
        'payback_convention', 'Payback(−100;50;50;50) = 3 periods', 'identity',
        'cumulative-cash convention (period count to positive cumulative)', 'period',
    ...
    def cec_sum() -> tuple[float, float, float]:
        ...
        c = out['components']
        return (c['clay_contribution'] + c['organic_matter_contribution'] + c['ph_factor'], out['cec'], 1e-9)
    checks.append(_run(
        'cec_component_sum', 'CEC equals sum of components', 'identity',
        'Brady & Weil 2017 (component model)', 'meq/100g',
        cec_sum,
    ))

    # 13 — FAO salinity class boundary
    checks.append(_run(
        'fao_salinity_class', 'EC 4 dS/m ⇒ moderately saline', 'reference',
    ...
    def carbon_bound() -> tuple[float, float, float]:
        out = calculate_carbon_sequestration(CarbonProjectType.AFFORESTATION, 100.0, 10, 'temperate')
        per_ha = float(out.get('annual_rate_tonnes', 0)) / 100.0
        return (0.0, per_ha, 20.0)
    checks.append(_run(
        'carbon_rate_bound', 'Afforestation ≤ 20 tCO2/ha/yr (IPCC plausibility)', 'reference',
        'IPCC AR6 biophysical plausibility bound', 'tCO2/ha/yr',
        carbon_bound,
