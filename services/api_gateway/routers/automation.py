...
class AgentRunRequest(BaseModel):
    ...
    watch_farms: list[int] = Field(
        default_factory=list, description="Farm ids to report satellite status for"
    )
...
def automation_health() -> dict[str, Any]:
    return {
        ...
        "token_configured": AGENT_TOKEN != "dev-agent-token",
    }
...
def agent_run(
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            runs = int(
                conn.exec_driver_sql("SELECT COUNT(*) FROM hydroma_model_runs").scalar() or 0
            )
        digest["database"] = {"journal_mode": journal, "model_runs": runs}
        digest["steps"].append(
            {"step": "database", "status": "ok", "detail": f"{runs} model runs stored"}
        )
    except Exception as exc:
    ...
    if payload.watch_farms:
        ...
        try:
            ...
            digest["steps"].append(
                {"step": "satellite_status", "status": "error", "detail": str(exc)}
            )
