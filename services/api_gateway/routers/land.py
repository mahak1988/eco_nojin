...
@router.post(
    "/profiles",
    operation_id="create_land_profile",
    response_model=LandProfile,
...
async def get_profile(profile_id: str):
    ...
    if profile is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Profile not found: {profile_id}"
        )
...
@router.delete(
    "/profiles/{profile_id}",
    operation_id="delete_land_profile",
...
@router.post(
    "/profiles/{profile_id}/terrain-analysis",
    operation_id="analyze_land_terrain",
    response_model=TerrainAnalysis,
)
async def analyze_terrain(profile_id: str, request: AnalyzeTerrainRequest):
    ...
    try:
        ...
        analysis = land_service.analyze_terrain(
            profile_id=profile_id, dem_array=dem_array, resolution=request.resolution
        )
...
@router.post(
    "/profiles/{profile_id}/drainage-analysis",
    operation_id="analyze_land_drainage",
    response_model=DrainageAnalysis,
)
...
@router.post(
    "/profiles/{profile_id}/capability-assessment",
    operation_id="assess_land_capability",
