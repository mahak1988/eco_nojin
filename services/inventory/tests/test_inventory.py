...
class TestStockService:
    ...
    async def test_create_sku(self, stock_service):
        sku = await stock_service.create_sku("TEST-SKU-001", "Test Product", uom="kg", standard_cost=Decimal("10"))
    ...
    async def test_receipt_increases_stock(self, stock_service, sku):
        movement = await stock_service.receipt(
            sku_id=sku.id, warehouse_id=stock_service._default_wh.id,
            qty=Decimal("100"), created_by="test-user",
        )
    ...
    async def test_issue_decreases_stock(self, stock_service, sku):
        await stock_service.receipt(sku.id, stock_service._default_wh.id, Decimal("100"), created_by="test-user")
        movement = await stock_service.issue(sku.id, stock_service._default_wh.id, Decimal("30"), created_by="test-user")
    ...
    async def test_issue_insufficient_stock_fails(self, stock_service, sku):
        await stock_service.receipt(sku.id, stock_service._default_wh.id, Decimal("10"), created_by="test-user")
        with pytest.raises(EcoNojinException) as exc_info:
            await stock_service.issue(sku.id, stock_service._default_wh.id, Decimal("50"), created_by="test-user")
    ...
    async def test_transfer_stock(self, stock_service, sku):
        ...
        await stock_service.receipt(sku.id, stock_service._default_wh.id, Decimal("100"), created_by="test-user")
        movement = await stock_service.transfer(sku.id, stock_service._default_wh.id, wh2.id, Decimal("40"), created_by="test-user")
    ...
    async def test_adjust_stock(self, stock_service, sku):
        await stock_service.receipt(sku.id, stock_service._default_wh.id, Decimal("50"), created_by="test-user")
        await stock_service.adjust(sku.id, stock_service._default_wh.id, Decimal("-5"), reason="damaged", created_by="admin")
        balance = await stock_service.get_balance(sku.id, stock_service._default_wh.id)
        assert balance.on_hand == Decimal("45")

    async def test_scrap_reduces_stock(self, stock_service, sku):
        await stock_service.receipt(sku.id, stock_service._default_wh.id, Decimal("50"), created_by="test-user")
        await stock_service.scrap(sku.id, stock_service._default_wh.id, Decimal("10"), reason="damaged", created_by="admin")
        balance = await stock_service.get_balance(sku.id, stock_service._default_wh.id)
        assert balance.on_hand == Decimal("40")

    async def test_return_increases_stock(self, stock_service, sku):
        await stock_service.receipt(sku.id, stock_service._default_wh.id, Decimal("50"), created_by="test-user")
        await stock_service.issue(sku.id, stock_service._default_wh.id, Decimal("20"), created_by="test-user")
        await stock_service.return_goods(sku.id, stock_service._default_wh.id, Decimal("10"), created_by="test-user")
        balance = await stock_service.get_balance(sku.id, stock_service._default_wh.id)
        assert balance.on_hand == Decimal("40")
    ...
    async def test_reservation_blocks_stock(self, stock_service, sku):
        ...
        reservation = await stock_service.reserve_stock(
            sku_id=sku.id, warehouse_id=stock_service._default_wh.id,
            qty=Decimal("50"), reference_type="order", reference_id="order-1",
    ...
    async def test_reservation_insufficient_available_fails(self, stock_service, sku):
        await stock_service.receipt(sku.id, stock_service._default_wh.id, Decimal("30"), created_by="test-user")
        await stock_service.reserve_stock(
            sku_id=sku.id, warehouse_id=stock_service._default_wh.id,
            qty=Decimal("20"), reference_type="order", reference_id="order-1",
            created_by="test-user",
        )
        with pytest.raises(EcoNojinException) as exc_info:
            await stock_service.reserve_stock(
                sku_id=sku.id, warehouse_id=stock_service._default_wh.id,
                qty=Decimal("50"), reference_type="order", reference_id="order-2",
                created_by="test-user",
            )
