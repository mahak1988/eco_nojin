...
def pq_sign(private_key_pem: bytes, data: bytes) -> bytes:
    ...
    key = serial.load_pem_private_key(private_key_pem, password=None)
...
def pq_verify(public_key_pem: bytes, data: bytes, signature: bytes) -> bool:
    ...
    serial = serialization
    try:
        key = serial.load_pem_public_key(public_key_pem)
...
def pq_public_key(private_key_pem: bytes) -> bytes:
    ...
    return key.public_key().public_bytes(
        encoding=serial.Encoding.PEM, format=serial.PublicFormat.SubjectPublicKeyInfo
    )
...
def generate_ml_kem_keys() -> Tuple[bytes, bytes]:
    ...
    pub = key.public_key().public_bytes(encoding=serial.Encoding.PEM, format=serial.PublicFormat.SubjectPublicKeyInfo)
    return priv, pub
