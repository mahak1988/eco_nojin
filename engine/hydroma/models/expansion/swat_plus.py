...
class SWATPlusInputs:
    ...
    management_schedule: Optional[Dict] = None
    
    # Climate
    climate_dir: Optional[Path] = None
    weather_stations: Optional[List[Dict]] = None
    
    # Hydrology
    stream_network: Optional[Path] = None
    reservoirs: Optional[List[Dict]] = None
    point_sources: Optional[List[Dict]] = None
    
    # Calibration
    observed_flow: Optional[pd.DataFrame] = None  # date, flow_cms
    observed_sediment: Optional[pd.DataFrame] = None
    observed_nutrients: Optional[pd.DataFrame] = None
...
class SWATPlusOutputs:
    ...
    pesticides: Optional[pd.DataFrame] = None
    
    # Crop yields
    crop_yields: Optional[pd.DataFrame] = None
...
class SWATPlusModel(ScientificModel):
    ...
    def __init__(
        ...
        default_project_dir: Optional[Path] = None,
        config: Optional[SWATPlusConfig] = None,
    ):
        """
        ...
        self._last_run_outputs: Optional[SWATPlusOutputs] = None
...
"""
    ...
    def _parse_outputs(self, project_dir: Path) -> SWATPlusOutputs:
        ...
        return SWATPlusOutputs(
            ...
            summary={
                ...
                "mean_annual_flow": float(streamflow.groupby("year")["flow_cms"].sum().mean()) if not streamflow.empty else 0,
            },
            nse=nse,
        )
    ...
    def _calculate_nse(self, simulated: pd.DataFrame) -> Optional[float]:
    ...
    def _outputs_to_dict(self, outputs: SWATPlusOutputs) -> Dict[str, Any]:
        ...
        return {
            "streamflow": outputs.streamflow.to_dict("records") if not outputs.streamflow.empty else [],
            ...
            "phosphorus": outputs.phosphorus.to_dict("records") if not outputs.phosphorus.empty else [],
