...
class GroundwaterInput(BaseModel):
    ...
    observation_distance_m: float = Field(
        ..., gt=0, description="Distance from well to observation point (m)"
    )
...
class GroundwaterOutput(BaseModel):
    ...
    drawdown_m: float | Any | None = Field(
        None, description="Calculated drawdown (m) - Can be a scalar or xr.DataArray"
