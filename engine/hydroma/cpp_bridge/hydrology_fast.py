...
def compute_wave_parameters(
    ...
    # h = (q * n / sqrt(S))^(3/5)
    h_normal = (q * manning_n / np.sqrt(bed_slope)) ** 0.6 if q > 0 and bed_slope > 0 else 0.1
...
def route_flood_wave(
    ...
    params = compute_wave_parameters(channel_length, bed_slope, manning_n, channel_width, peak_in)

    # Single-reach Muskingum-Cunge routing
    outflow = _muskingum_cunge_route(inflow, params["K"], params["x"], dt)
...
def route_multi_reach(
    ...
    for _ in range(n_reaches):
        params = compute_wave_parameters(
            reach_length, bed_slope, manning_n, channel_width, float(np.max(current_inflow))
        )
        total_K += params["K"]

        current_inflow = _muskingum_cunge_route(current_inflow, params["K"], params["x"], dt)
