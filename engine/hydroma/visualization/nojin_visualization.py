...
class ProjectMapper:
    ...
    def create_heatmap_data(
        self, projects: list[dict], weight_field: str = "carbon_sequestered_t"
    ) -> dict:
        points = []
        for proj in projects:
            points.append(
...
class EconomicChartBuilder:
    ...
    def create_cost_benefit_breakdown(self, breakdown: dict) -> ChartConfig:
        ...
        dataset = ChartDataset(
            label="Cost/Benefit Breakdown",
            data=values,
            borderColor="#ffffff",
            backgroundColor="rgba(59, 130, 246, 0.7)",
        )
        return ChartConfig(
            type="doughnut", labels=labels, datasets=[dataset], title="Cost vs Benefit Breakdown"
        )
...
class WaterVisualization:
    def create_water_savings_chart(self, monthly_data: list[dict]) -> ChartConfig:
        ...
        return ChartConfig(
            type="bar",
            labels=labels,
            datasets=[baseline_dataset, optimized_dataset],
            title="Monthly Water Usage Comparison",
            x_label="Month",
            y_label="Water (m3/ha)",
        )
