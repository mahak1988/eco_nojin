...
class SoilMoistureOperator(ObservationOperator):
    ...
    def __init__(self, depth_weights: Optional[np.ndarray] = None):
...
class StreamflowOperator(ObservationOperator):
    ...
    def __init__(self, routing_model: Optional[Callable] = None):
...
class EnsembleKalmanFilter:
    ...
    def initialize_ensemble(
        ...
        logger.info(f"Initialized ensemble: {self.state_dim} state vars, {self.ensemble_size} members")
    
    def forecast_step(self, dt: float, forcing: Optional[Dict] = None) -> None:
        ...
        for i in range(self.ensemble_size):
            ...
            # Run model
            input_data = type('ModelInput', (), {'get': lambda self, k, d=None: state_dict.get(k, d)})()
    ...
    def analysis_step(self, observations: List[Observation]) -> np.ndarray:
        ...
        for i, obs in enumerate(observations):
            op = self.obs_operators.get(obs.variable, LinearObservationOperator(np.eye(self.state_dim)))
        ...
        if self.config.method == "EnKF":
            y_perturbed = y_obs[:, np.newaxis] + np.random.randn(n_obs, n_ens) * np.sqrt(np.diag(R))[:, np.newaxis]
...
class Variational4DVar:
    ...
    def cost_function(self, x: np.ndarray, observations: List[Observation], xb: np.ndarray) -> float:
    ...
    def cost_gradient(self, x: np.ndarray, observations: List[Observation], xb: np.ndarray) -> np.ndarray:
    ...
    def analysis_step(self, observations: List[Observation]) -> np.ndarray:
        ...
        for i in range(self.config.ensemble_size):
            ...
            for obs in observations:
                ...
                # Gaussian likelihood
                log_like += -0.5 * ((Hx - obs.value) / obs.error_std) ** 2 - 0.5 * np.log(2 * np.pi * obs.error_std ** 2)
...
class HybridEnKF4DVar:
    ...
    def assimilate(
        ...
        static_B: Optional[np.ndarray] = None,
        ...
        for t, obs in enumerate(observations_sequence):
            ...
            hybrid_B = (1 - self.hybrid_weight) * B + self.hybrid_weight * P_ens
