...
class CalibrationInput(BaseModel):
    ...
    model_runner: Any = Field(
        ...,
        description="Instance of the model to calibrate (must follow ModelRunner protocol) - Type erased for Pydantic compatibility",
    )  # Changed type hint to Any
    ...
    parameter_bounds: dict[str, tuple[float, float]] = Field(
        ..., description="Dictionary of parameter names and their (min, max) bounds"
    )
    objective_function: str = Field(
        "rmse", description="Objective function to minimize ('rmse', 'nse', etc.)"
    )
...
class CalibrationOutput(BaseModel):
    ...
    calibrated_parameters: dict[str, float] = Field(
        ..., description="Final calibrated parameter values"
    )
    best_objective_value: float = Field(
        ..., description="Value of the objective function at the optimum"
    )
    history: list[dict[str, Any]] = Field(
        ..., description="History of parameter sets and objective values during calibration"
    )
...
class Calibrator:
    ...
    def _simple_brute_force(
        self, input_data: CalibrationInput, num_steps: int = 10
    ) -> CalibrationOutput:
        ...
        if len(param_names) == 2:
            ...
            for i in range(num_steps + 1):
                for j in range(num_steps + 1):
                    ...
                    try:
                        ...
                        model_output = input_data.model_runner.run(
                            params
                        )  # Runtime check ensures compatibility
                        ...
                        if input_data.objective_function == "rmse":
                            ...
                            raise ValueError(
                                f"Unsupported objective function: {input_data.objective_function}"
                            )
            ...
            raise NotImplementedError(
                "Brute force calibration is only implemented for 1 or 2 parameters. Use an external optimizer for more."
            )
