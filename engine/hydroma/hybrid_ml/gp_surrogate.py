...
class GPConfig:
    ...
    noise_prior: Optional[Tuple[float, float]] = None  # (concentration, rate) for Gamma prior
...
if GPYTORCH_AVAILABLE:
    class ExactGPModel(gpytorch.models.ExactGP):
        ...
        def __init__(
            ...
            if kernel_type == "rbf":
                base_kernel = gpytorch.kernels.RBFKernel(ard_num_dims=train_x.shape[1] if ard else None)
            elif kernel_type == "matern":
                base_kernel = gpytorch.kernels.MaternKernel(nu=nu, ard_num_dims=train_x.shape[1] if ard else None)
            elif kernel_type == "rq":
                base_kernel = gpytorch.kernels.RQKernel(ard_num_dims=train_x.shape[1] if ard else None)
            elif kernel_type == "spectral":
                base_kernel = gpytorch.kernels.SpectralMixtureKernel(num_mixtures=4, ard_num_dims=train_x.shape[1] if ard else None)
            else:
                base_kernel = gpytorch.kernels.MaternKernel(nu=nu, ard_num_dims=train_x.shape[1] if ard else None)
...
class GPSurrogateModel(ScientificModel):
    ...
    def __init__(
        self,
        base_model: Optional[ScientificModel] = None,
        ...
        self._model: Optional[ExactGPModel] = None
            ...
            if query_points is not None:
                if isinstance(query_points, np.ndarray):
                    if query_points.shape[1] != self.input_dim:
                        raise ValueError(f"Expected {self.input_dim} input dimensions, got {query_points.shape[1]}")
        return True
    ...
    def train_on_data(
        ...
        self._model = ExactGPModel(
            X_tensor, y_tensor, self._likelihood,
    ...
    def _optimize(self) -> Dict[str, Any]:
        ...
        return {
            "final_loss": losses[-1],
            "loss_history": losses,
            "lengthscales": self._model.covar_module.base_kernel.lengthscale.detach().cpu().numpy().tolist(),
    ...
    def train_from_base_model(
        ...
        for i, x in enumerate(X):
            ...
            try:
                input_data = ModelInput(**dict(zip([f"param_{j}" for j in range(len(x))], x)))
                output = self.base_model.compute(input_data)
                if output.success:
                    # Extract scalar output
                    if isinstance(output.outputs, dict):
                        val = list(output.outputs.values())[0]
    ...
    def predict_with_gradients(
        ...
        X_tensor = torch.tensor(X, dtype=torch.float32, device=self.config.device, requires_grad=True)
        ...
        with gpytorch.settings.fast_pred_var():
            ...
            gradients = torch.autograd.grad(
                mean.sum(), X_tensor, create_graph=True
            )[0]
    ...
    def uncertainty_quantification(self, inputs: ModelInput, n_samples: int = 100) -> Dict[str, Any]:
