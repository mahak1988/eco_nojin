...
def validate_index_robustness(
    grid_size: int = 64, seed: int = 42, corruptions: list[CorruptionConfig] | None = None
    ...
    if corruptions is None:
        corruptions = [
            CorruptionConfig(
                missing_band=False, cloud_cover_pct=10.0, sensor_noise_std=0.02, seed=seed
            ),
            CorruptionConfig(cloud_cover_pct=50.0, dead_pixels_pct=5.0, seed=seed + 1),
            CorruptionConfig(
                cloud_cover_pct=90.0, sensor_noise_std=0.05, dead_pixels_pct=10.0, seed=seed + 2
            ),
        ]
    ...
    for cfg in corruptions:
        ...
        in_range = (
            ndvi.min() >= -1.0 and ndvi.max() <= 1.0 and nbr.min() >= -1.0 and nbr.max() <= 1.0
        )
...
def validate_fallbacks() -> list[StressTestResult]:
    ...
    r = SRT(
        test_name="numba_fallback_ndvi",
        scenario="fallback",
        passed=bool(np.all(np.isfinite(a))),
        detail=f"ndvi_fast finite={np.all(np.isfinite(a))}",
        metric=float(np.nanmean(a)),
    ...
    results.append(
        SRT(
            test_name="wrapper_scalar_fallback",
            scenario="fallback",
            passed=bool(ok),
            detail=f"ndvi={ndvi:.3f} evi={evi:.3f} savi={savi:.3f}",
            metric=float(np.mean(list(allidx.values()))),
            metadata=_meta("wrapper_fallback", 0, {}),
        )
    )
    ...
    results.append(
        SRT(
            test_name="cpp_availability",
            scenario="fallback",
            passed=True,
            detail=f"cpp_available={cpp_available}; python fallback used regardless",
            metric=float(cpp_available),
            metadata=_meta("cpp_availability", 0, {}),
        )
    )
    return results
...
def run_combinatorial(
    climate_scenarios: list[dict[str, Any]],
    ...
    for ci, clim in enumerate(climate_scenarios):
        for di, dis in enumerate(disaster_scenarios):
            ...
            m_scen = MonsoonScenario(
                seasonal_total_mm=clim.get("seasonal_total_mm", 800.0)
                * clim.get("precip_factor", 1.0),
            ...
            if dtype == "wildfire":
                dres = simulate_wildfire(
                    FireScenario(
                        grid_size=64,
                        fire_radius_cells=dis.get("radius", 30),
                        intensity=dis.get("intensity", 1.0),
                        seed=sid,
                    )
                )
                metric = dres.dNBR
                ...
                dres = simulate_flood(
                    DSFloodScenario(
                        grid_size=64,
                        rainfall_24h_mm=dis.get("rainfall", 120.0),
                        curve_number=dis.get("cn", 82.0),
                        seed=sid,
                    )
                )
                ...
                dres = simulate_storm(
                    StormScenario(max_wind_speed_kmh=dis.get("wind", 150.0), seed=sid)
                )
                metric = dres.wind_damage_index
            else:
                dres = simulate_landslide(
                    LandslideScenario(
                        slope_pct=dis.get("slope", 25.0),
                        rainfall_24h_mm=dis.get("rainfall", 90.0),
                        seed=sid,
                    )
                )
