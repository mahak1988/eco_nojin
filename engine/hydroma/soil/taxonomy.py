...
def classify_usda_texture(clay: float, silt: float, sand: float) -> dict:
    ...
    result = {
        "texture": texture,
        "texture_details": {"clay_percent": clay, "silt_percent": silt, "sand_percent": sand},
        **properties,
    }
...
def _determine_texture_class(clay: float, silt: float, sand: float) -> str:
    ...
    for texture, ranges in TEXTURE_CLASSES.items():
        ...
        if (
            clay_range[0] <= clay <= clay_range[1]
            and silt_range[0] <= silt <= silt_range[1]
            and sand_range[0] <= sand <= sand_range[1]
        ):
            return texture
...
def _calculate_texture_properties(texture: str, clay: float, silt: float, sand: float) -> dict:
    ...
    permeability_values = {
        "sand": "very_high",
        "loamy_sand": "high",
        "sandy_loam": "moderate_high",
        "loam": "moderate",
        "silt_loam": "moderate_low",
        "silt": "low",
        "sandy_clay_loam": "moderate_low",
        "clay_loam": "low",
        "silty_clay_loam": "very_low",
        "sandy_clay": "low",
        "silty_clay": "very_low",
        "clay": "very_low",
    }
    ...
    erodibility_values = {
        ...
        "silt_loam": 0.35,
        "silt": 0.40,
        "sandy_clay_loam": 0.25,
        "clay_loam": 0.30,
        "silty_clay_loam": 0.32,
        "sandy_clay": 0.28,
        "silty_clay": 0.35,
        "clay": 0.38,
    }
...
def _estimate_drainage(texture: str) -> str:
    ...
    drainage_map = {
        "sand": "excessive",
        "loamy_sand": "excessive",
        "sandy_loam": "well",
        "loam": "well",
        "silt_loam": "moderate",
        "silt": "poor",
        "sandy_clay_loam": "moderate",
        "clay_loam": "poor",
        "silty_clay_loam": "poor",
        "sandy_clay": "poor",
        "silty_clay": "very_poor",
        "clay": "very_poor",
    }
...
def _estimate_workability(texture: str) -> str:
    ...
    workability_map = {
        ...
        "sandy_clay_loam": "moderate",
        "clay_loam": "difficult",
        "silty_clay_loam": "difficult",
        "sandy_clay": "difficult",
        "silty_clay": "very_difficult",
        "clay": "very_difficult",
    }
    return workability_map.get(texture, "moderate")


def get_soil_taxonomy(
    clay: float,
    silt: float,
    sand: float,
    ...
    return {
        ...
        "taxonomy": {
            ...
            "classification_basis": {"organic_matter": organic_matter, "ph": ph, "cec": cec},
        },
        "interpretation": _interpret_taxonomy(soil_order, texture_result["texture"]),
    }


def _determine_soil_order(organic_matter: float, ph: float, cec: float | None = None) -> str:
