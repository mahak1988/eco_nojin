...
class SoilProfile(Base):
    ...
    ec: Mapped[float | None] = mapped_column(Float, nullable=True, comment="Electrical Conductivity (dS/m)")
...
class Plant(Base):
    ...
    category: Mapped[str] = mapped_column(String(50), nullable=False, comment="e.g., crop, tree, medicinal")
...
class Material(Base):
    ...
    c_n_ratio: Mapped[float | None] = mapped_column(Float, nullable=True, comment="Carbon to Nitrogen ratio")
    ph: Mapped[float | None] = mapped_column(Float, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
