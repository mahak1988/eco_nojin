...
class SeedOptimizationEngine:
    ...
    def h16_field_hardiness(
        ...
        score = (
            w["acclimatization"] * acclimatization_score
    ...
    # ------------------------------------------------------------------- H17
    def h17_native_resilience(
        self,
        local_adaptation_years: int,
        ...
        score = (
            w["local_adaptation"] * adaptation_score
    ...
    # ------------------------------------------------------------------- H18
    def h18_growth_duration_optimizer(
        self,
    ...
    def generate_seed_advisory(
        ...
        # H15: تطبیق ژنوتیپ-محیط
        advisory["h15_gxe"] = self.h15_gxe_matching(variety_tolerances, site_stress_profile)

        # H16: سازگاری میدانی (فقط برای کشت بافت)
        if is_tissue_culture:
            advisory["h16_hardiness"] = self.h16_field_hardiness(
                acclimatization_score=0.4, root_quality_score=0.5, defense_score=0.3
            )
        ...
        # H19: آسیب‌پذیری ژنتیکی
        advisory["h19_vulnerability"] = self.h19_genetic_vulnerability(genetic_diversity, 60.0, 3)
