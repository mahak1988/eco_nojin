...
YieldStats yield_ensemble_lhs(double mean_water_mm, double water_std_mm,
    ...
    yields.reserve(n_samples);
    for (const auto& s : unit) {
        // Truncated normal via Box-Muller on uniform inputs.
        std::mt19937_64 rng(seed + static_cast<unsigned long long>(s[0] * 1e6));
        ...
        yields.push_back(simplified_yield(water, temp, crop));
    }
