...
| تست‌های بک‌اند | **۲۵۴ مورد جمع‌آوری‌شده** | نتیجه: **۵۷ fail / ۱۱۸ pass / ۷۹ error** |
| تست فرانت‌اند | **۰** | هیچ فایل تست/playwright در `apps/web` وجود ندارد |
| خطای TypeScript فرانت | **۶ خطا در ۵ فایل** | build فرانت‌اند شکست می‌خورد |
...
### ۳.۱ 🔴 تست بک‌اند قرمز است — بزرگ‌ترین ریسک پروژه
اجرای واقعی: `pytest -q` →
```
57 failed, 118 passed, 79 errors
ERROR services/carbon/tests/test_carbon_credit.py  (ImportError: cannot import name 'MotorOutput')
```
- **ریشهٔ اصلی (~۱۹۰ تست از ۲۵۴):** یک mapper شکسته در SQLAlchemy:
  `Mapper[ComputeJob(compute_jobs)]` → خطای initialization، که به‌صورت آبشاری همهٔ mapperها را از کار می‌اندازد.
- این دقیقاً همان آیتم `S1-01 (JobEvent missing)` در `EXECUTION_PLAN_FA.md` و ادعای «Fixed» در `docs/11` است؛ ولی در کد **هنوز رفع نشده**.
- ادعاهای اسناد («۳۲۷ pass»، «۲۶۹ pass / ۲ fail»، «۷۹ pass») با واقعیت امروز ناسازگار است و به‌عنوان سنجهٔ کیفیت قابل اتکا نیست.

### ۳.۲ 🔴 فرانت‌اند build نمی‌شود
`tsc --noEmit` → ۶ خطا در ۵ فایل:
| فایل | خطا |
|---|---|
| `src/components/index.ts` | `Cannot find module './ui'` — پوشهٔ `components/ui/` **خالی** است ولی barrel آن را export می‌کند |
| `src/components/admin/index.ts` | `Cannot find module './AdminLayout'` — فایل در `components/layout/` است |
| `src/components/layout/AdminLayout.tsx:268` | `Cannot find name 'handleLogout'` (کد کپی‌شده و ناتمام) |
| `AdminLayout.tsx:297` | پراپ `className` به کامپوننت بدون آن پاس شده |
| `src/components/providers/QueryProvider.tsx` | `Cannot find name 'hydrate'` + نقض قرارداد `Persister` (باگ واقعی persistence آفلاین) |
| `src/lib/offline/sync.ts:53` | `TS18046: 'registration.sync' is of type 'unknown'` |
هیچ `ignoreBuildErrors` هم تنظیم نشده ⇒ `next build` شکست می‌خورد.

### ۳.۳ 🔴 صفر تست فرانت‌اند
هیچ `*.test.tsx`، هیچ پوشهٔ `tests/`، هیچ `playwright.config`. اما `package.json` اسکریپت‌های `test` و `test:e2e` را تبلیغ می‌کند ⇒ خط لولهٔ کیفیت فرانت‌اند توخالی است.
...
```powershell
# بک‌اند
.venv\Scripts\python.exe -m pytest -q                      # 0 failed, 0 error
...
pnpm -C apps/web test                                      # all pass (پس از F3)
pnpm -C apps/web test:e2e                                  # 5 flow سبز
...
1. رفع mapper شکستهٔ `ComputeJob`/`JobEvent` و سبز کردن `pytest` (تنها یک ریشه، ~۱۹۰ تست).
2. رفع ۶ خطای TypeScript و راه‌اندازی `type-check`/`build` به‌عنوان گیت اجباری.
3. اجرای یک «ممیزی حقیقت»: علامت‌گذاری مجدد وضعیت‌ها در `docs/11` و اصلاح درفت `frontend/` → `apps/web` تا تصمیم‌ها روی دادهٔ درست گرفته شود.
