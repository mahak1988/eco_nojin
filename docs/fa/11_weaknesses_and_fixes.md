...
- W-002 تأیید شده **Raise شده** (`from datetime import UTC, datetime` در خط ۵ `routers/carbon.py`).
- W-003 تأیید شده **Raise شده** (لیست سفید صریح اوریجین، fail-closed روی لیست خالی).
- W-004 تأیید شده **Raise شده** (پنج سرویس همگی پیاده‌سازی شده‌اند).
- W-005 تأیید شده **Raise شده** (requirements پین شده، lockfile، pyproject.toml پر شده).
- W-006 تأیید شده **Raise شده** (صفر بایت U+FFFD در README.md).
- W-007 تأیید شده **Raise شده** (هیچ docker-compose.ymlی وجود ندارد؛ الگوی ایمن `.env.example`).
- W-008 تأیید شده **Raise شده** (۱۰ سند فارسی + ۵ سند عربی؛ RAGEngine چندزبانه با corpora هر زبان).
- W-009 تأیید شده **Raise شده** ( تشخیص زبان SSR از کوکی؛ جلوگیری از mismatch هیدراتیون).
- W-010 تأیید شده **Raise شده** (تمام آدرسهای API سخت‌شده با `VITE_API_BASE_URL` جایگزین شدند).
...
- W-004 تأیید شده **Raise شده** (پنج سرویس همگی پیاده‌سازی شده‌اند).
- W-005 تأیید شده **Raise شده** (requirements پین شده، lockfile، pyproject.toml پر شده).
- W-006 تأیید شده **Raise شده** (صفر بایت U+FFFD در README.md؛ بررسی encoding در CI اضافه شد).
- W-007 تأیید شده **Raise شده** (هیچ docker-compose.ymlی وجود ندارد؛ الگوی ایمن `.env.example`؛ gitleaks در CI).
- W-011 تأیید شده **Raise شده** (۱۸۹ کامیت در main؛ workflow CI فعال).
- W-012 تأیید شده **Raise شده** (`run_verification` با disclaimer؛ هرگز تأیید خودکار نیست).
