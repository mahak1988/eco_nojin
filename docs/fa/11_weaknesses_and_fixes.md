...
|---|---|---|
| **Raise شده** | **۱۰** | W-001 (داده ماهواره واقعی)، W-002 (Import datetime)، W-003 (لیست سفید CORS)، W-004 (داربست سرویس‌ها)، W-005 (پین نسخه‌ها)، W-006 (encdoing README)، W-007 (رازها)، W-011 (مخزن Git)، W-012 (راستی‌آزمایی کربن)، W-014 (Alembic) |
| در حال رفع | ۵ | W-008 (پیکره RAG فارسی)، W-009 (RTL در SSR)، W-010 (آدرس API)، W-013 (تست‌های عددی)، W-022 (پاکسازی type) |
| برنامه‌ریزی | ۵ | W-015 (تست audit trail)، W-016 (تست auth)، W-017 (تست TLS)، W-019 (تست persistence)، W-021 (ممیزی WCAG) |
