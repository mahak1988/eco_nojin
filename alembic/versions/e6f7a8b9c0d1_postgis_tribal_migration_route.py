...
def upgrade() -> None:
    ...
    op.add_column('marketplaces', sa.Column('coverage_radius_km', sa.Integer(), nullable=True, server_default='50'))
    op.add_column('marketplaces', sa.Column('accountability_score', sa.Numeric(precision=5, scale=2), nullable=True, server_default='100'))
    op.execute("ALTER TABLE marketplaces ADD COLUMN IF NOT EXISTS static_location geography(POINT, 4326)")
    op.execute("ALTER TABLE marketplaces ADD COLUMN IF NOT EXISTS migration_route geography(LINESTRING, 4326)")
    op.execute("ALTER TABLE marketplaces ADD COLUMN IF NOT EXISTS current_position geography(POINT, 4326)")
    op.execute('CREATE INDEX IF NOT EXISTS idx_marketplace_location ON marketplaces USING GIST (static_location)')
    op.execute('CREATE INDEX IF NOT EXISTS idx_marketplace_route ON marketplaces USING GIST (migration_route)')
...
def downgrade() -> None:
    ...
    for col in ('accountability_score', 'coverage_radius_km', 'village_code', 'city', 'province', 'postal_code'):
